...
function App(){
  ...
  return (<>
    ...
    {showDigest&&<div className="bsbg" onClick={()=>setSD(false)}><div className="bsm" onClick={e=>e.stopPropagation()}>
      ...
      {liveSigs.length>0?<div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>{liveSigs.slice(0,10).map(s=>{const cat=getCat(s.cat,lang);const co=cos.find(c=>{const n=s.company||"";return c.name.toLowerCase()===n.toLowerCase()||n.toLowerCase().includes(c.name.toLowerCase().split(" ")[0])});return (<div key={s.id} className="card" style={{padding:"12px 16px",cursor:"pointer"}} onClick={()=>{setSS(s);setSD(false)}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:13}}>{cat?.icon}</span>{co&&<span style={{fontSize:11,fontWeight:500,color:"var(--gold2)"}}>{co.name}</span>}</div><span className="badge" style={{background:sBg(s.imp),color:sT(s.imp)}}>{s.imp}</span></div><p style={{fontSize:12,color:"var(--t1)",lineHeight:1.4}}>{tx(s.title,lang)}</p><p style={{fontSize:10,color:"var(--t5)",marginTop:6}}>{tx(s.src||s.source,lang)} · {fD(s.at)}</p></div>)})}</div>:<div style={{textAlign:"center",padding:"32px 16px"}}><p style={{fontSize:13,color:"var(--t4)",marginBottom:8}}>{t("no_activity")}</p><button className="btn bp" style={{padding:"8px 18px",fontSize:12}} onClick={()=>{setSD(false);refreshSignals()}}><I.refresh/>{lang==="fr"?"Lancer la veille":"Start monitoring"}</button></div>}
      {/* Copy digest */}
