...
function Logo({name,sz=24,fallback}){const url=logoUrl(name);const[ok,setOk]=useState(true);if(!url||!ok)return <span className="mono" style={{width:sz,height:sz,fontSize:sz*.42}}>{fallback||(name||"?").charAt(0)}</span>;return <img src={url} alt="" style={{width:sz,height:sz,borderRadius:sz*.2,objectFit:"contain",background:"white",padding:1}} onError={()=>setOk(false)}/>}
