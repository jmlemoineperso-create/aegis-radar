...
function Logo({name,sz=24,fallback}){
  ...
  const mono=<span className="mono" style={{width:sz,height:sz,fontSize:sz*.42}}>{fallback||(name||"?").charAt(0)}</span>;
  if(!d||failed>=sources.length)return mono;
  return <img src={sources[failed]} alt="" style={{width:sz,height:sz,borderRadius:sz*.18,objectFit:"contain",background:"white",padding:1,flexShrink:0}} onLoad={(e)=>{const img=e.target;if(img.naturalWidth<20||img.naturalHeight<20)setFailed(f=>f+1)}} onError={()=>setFailed(f=>f+1)}/>;
}
