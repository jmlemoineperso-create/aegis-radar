...
// ═══════════════════════════════════════════════════════════════
// AIG Financial Lines Intelligence — Fully Bilingual (FR/EN)
...
const T={
  ...
  dedication:{en:"© 2026 AIG — Financial Lines Intelligence",fr:"© 2026 AIG — Financial Lines Intelligence"},
...
.hdr{position:sticky;top:0;z-index:50;background:#002B5C;border-bottom:1px solid #001E42;padding:14px 20px}
...
function App(){
    ...
    try{
      ...
      if(data.signals&&data.signals.length>0){
        ...
        if(crits.length>0&&typeof Notification!=="undefined"&&Notification.permission==="granted"){
          ...
          if(crits.length>1)new Notification("AIG FL Intelligence",{body:`${lang==="fr"?`+ ${crits.length-1} autre(s) signal(aux) critique(s)`:`+ ${crits.length-1} more critical signal(s)`}`,icon:"/icon-192.png",tag:"aigfl-batch"});
        }
        setLastRefresh(new Date().toISOString());
  ...
  if(step==="select"){const selCount=cos.filter(c=>c.prio).length;return (
    <div className="fi" style={{minHeight:"100vh",padding:"40px 20px 100px",background:"var(--bg)"}}>
      <div style={{textAlign:"center",marginBottom:32}}><I.radar style={{margin:"0 auto 12px",display:"block",color:"var(--gold)"}}/><h2 style={{fontSize:22,fontWeight:600,color:"var(--t1)",marginBottom:8}}>{t("select_companies")}</h2><p style={{fontSize:13,color:"var(--t3)",lineHeight:1.55}}>{t("select_companies_sub")}</p></div>
  ...
  const SigCard=({s,d=0})=>{const cat=getCat(s.cat,lang);const co=cos.find(c=>c.id===s.cid)||cos.find(c=>s.company&&(c.name.toLowerCase()===s.company.toLowerCase()||s.company.toLowerCase().includes(c.name.toLowerCase().split(" ")[0])));const imps=getImpsAll(s.id);return (
    <div className={`card fi ${d>0?`fi${Math.min(d,5)}`:""}`} style={{padding:"16px 18px",cursor:"pointer"}} onClick={()=>setSS(s)}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:6}}>{co&&<Logo name={co.name} sz={20} fallback={co.logo}/>}<span style={{fontSize:11,fontWeight:500,color:"var(--gold2)"}}>{co?.name}</span><span style={{fontSize:10,color:"var(--t5)"}}>·</span><span style={{fontSize:10,color:"var(--t5)"}}>{tx(s.src,lang)}</span></div><span style={{fontSize:10,color:"var(--t5)",display:"flex",alignItems:"center",gap:4}}><I.clock/>{fD(s.at)}</span></div>
    ...
    return (<div className="bsbg" onClick={onClose}><div className="bsm" onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:"flex",gap:8,marginTop:4}}>
        ...
        <button className="btn" style={{flex:1,height:44,background:"var(--bg3)",color:"var(--t2)",border:"1px solid var(--b)",borderRadius:"var(--rs)",fontSize:12,fontWeight:600}} onClick={()=>{
          ...
          <div class="header"><div><span class="logo">AIG</span><br><span style="font-size:10px;text-transform:uppercase;letter-spacing:0.12em;color:#9ca3af">Financial Lines Intelligence</span></div><span class="date">${new Date().toLocaleDateString(lang==="fr"?"fr-FR":"en-GB",{day:"numeric",month:"long",year:"numeric"})}</span></div>
  ...
  const render=()=>{
    ...
    if(tab==="dashboard")return (<div style={{paddingBottom:100}}>
      ...
      <div style={{padding:"18px 20px"}}><p style={{fontSize:16,fontWeight:500,color:"var(--t1)",marginBottom:4}}>{greeting()}</p><p style={{fontSize:12,color:"var(--t4)",marginBottom:16}}>{t("dashboard_sub")}</p>
        ...
        {watched.length>0&&<div className="fi fi2" style={{marginBottom:22}}>
          ...
          {(()=>{const buckets=[{l:lang==="fr"?"Critique":"Critical",c:"#EF4444",min:75,max:100},{l:lang==="fr"?"Élevé":"High",c:"#F59E0B",min:50,max:74},{l:lang==="fr"?"Moyen":"Medium",c:"#3B82F6",min:25,max:49},{l:lang==="fr"?"Faible":"Low",c:"#16A34A",min:0,max:24}];const counts=buckets.map(b=>({...b,n:watched.filter(c=>(c.risk||50)>=b.min&&(c.risk||50)<=b.max).length}));const total=watched.length||1;return (
    ...
    if(tab==="watchlist")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><h2 style={{fontSize:18,fontWeight:700,color:"var(--t1)"}}>{t("watchlist_title")}</h2><p style={{fontSize:12,color:"var(--t4)",marginTop:4}}>{t("watchlist_sub")}</p></div>
      <div style={{padding:"18px 20px"}}>
        ...
        {sorted.map((c,i)=>{const sc=getSigs(c.id).length;const nc=getNotes(c.id).length;const lines=getLinesAll(getSigs(c.id));return (
          <div key={c.id} className={`fi fi${Math.min(i+1,5)}`} style={{padding:"10px 16px",borderBottom:"1px solid var(--b)",background:i%2===0?"transparent":"rgba(13,20,36,.4)",display:"grid",gridTemplateColumns:"32px 1fr 80px 50px 50px 36px 22px",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>setSC(c.id)}>
              ...
              <button style={{width:20,height:20,borderRadius:5,background:"rgba(239,68,68,.08)",border:"1px solid rgba(220,38,38,.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={e=>{e.stopPropagation();togW(c.id)}}><I.x style={{width:9,height:9,color:"#F87171"}}/></button>
          </div>)})}
    ...
    if(tab==="signals")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{fontSize:18,fontWeight:700,color:"var(--t1)"}}>{t("all_signals")}</h2><button className="bi" style={{width:34,height:34}} onClick={()=>{setSSh(!showSearch);if(showSearch)setSrch("")}}>{showSearch?<I.x/>:<I.filter/>}</button></div>{showSearch&&<input className="inp" style={{marginBottom:12}} placeholder={t("filter_signals")} value={search} onChange={e=>setSrch(e.target.value)} autoFocus/>}<div className="hsb" style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}><button className={`chip ${!activeCat?"on":""}`} onClick={()=>setACat(null)}>{t("all")}</button>{CATS.map(c=>{const cc=getCat(c.id,lang);return <button key={c.id} className={`chip ${activeCat===c.id?"on":""}`} onClick={()=>setACat(activeCat===c.id?null:c.id)}>{cc?.icon} {cc?.s}</button>})}</div></div>
    ...
    if(tab==="notes")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><h2 style={{fontSize:18,fontWeight:700,color:"var(--t1)"}}>{t("notes_title")}</h2><button className="btn bp" style={{padding:"6px 14px",fontSize:12}} onClick={()=>setSNN(true)}><I.plus/>{t("new_note")}</button></div></div>
    ...
    if(tab==="brief")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><h2 style={{fontSize:18,fontWeight:700,color:"var(--t1)"}}>{t("brief_title")}</h2><p style={{fontSize:12,color:"var(--t4)",marginTop:4}}>{t("brief_sub")}</p></div><button className="btn bp" style={{padding:"7px 14px",fontSize:12}} onClick={()=>setSNM(true)}><I.calendar/>{t("add_meeting")}</button></div></div>
      <div style={{padding:"18px 20px"}}>
        {/* Upcoming meetings */}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>{upcomingMtgs.map((mtg,i)=>{const co=cos.find(c=>c.id===mtg.cid);const ml=mtgLabel(mtg);const d=new Date(mtg.date);return (
          <div key={mtg.id} className={`card fi fi${Math.min(i+1,5)}`} style={{padding:"16px 18px",borderLeft:`3px solid ${ml.c}`,position:"relative"}}>
            <button style={{position:"absolute",top:8,right:8,width:20,height:20,borderRadius:5,background:"rgba(239,68,68,.08)",border:"1px solid rgba(220,38,38,.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={()=>deleteMeeting(mtg.id)}><I.x style={{width:10,height:10,color:"#F87171"}}/></button>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:24}}>{pastMtgs.slice(0,5).map(mtg=>{const co=cos.find(c=>c.id===mtg.cid);return (
          <div key={mtg.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 14px",borderBottom:"1px solid var(--b)",opacity:.6}}>
            ...
            <button style={{width:18,height:18,borderRadius:4,background:"rgba(239,68,68,.06)",border:"1px solid rgba(220,38,38,.08)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={()=>deleteMeeting(mtg.id)}><I.x style={{width:9,height:9,color:"#F87171"}}/></button>
          </div>)})}</div></>}
    ...
    if(tab==="settings")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><h2 style={{fontSize:18,fontWeight:700,color:"var(--t1)"}}>{t("settings_title")}</h2></div>
      <div style={{padding:"24px 20px"}}>
        ...
        <p style={{textAlign:"center",fontSize:11,color:"var(--t5)",marginBottom:28}}>© 2026 AIG — Financial Lines Intelligence</p>
  ...
  return (<>
    ...
    {showNewMeeting&&<div className="bsbg" onClick={()=>{if(mtgDictating)stopMtgDict();setSNM(false)}}><div className="bsm" onClick={e=>e.stopPropagation()}>
      ...
      <div style={{marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><label className="lbl" style={{color:"var(--t4)"}}>{t("meeting_notes")}</label><button className="btn" style={{padding:"4px 12px",fontSize:11,borderRadius:16,background:mtgDictating?"rgba(220,38,38,.12)":"rgba(0,114,206,.1)",color:mtgDictating?"#991B1B":"var(--gold)",border:`1px solid ${mtgDictating?"rgba(239,68,68,.3)":"rgba(0,114,206,.2)"}`}} onClick={mtgDictating?stopMtgDict:startMtgDict}>{mtgDictating?<><div style={{width:6,height:6,borderRadius:"50%",background:"#EF4444",animation:"pd 1s ease-in-out infinite",marginRight:4}}/>{lang==="fr"?"Arrêter":"Stop"}</>:<><I.mic style={{width:14,height:14}}/>{lang==="fr"?"Dicter":"Dictate"}</>}</button></div>{mtgDictating&&<p style={{fontSize:10,color:"#991B1B",marginBottom:6}}>{lang==="fr"?"Parlez, la note se rédige...":"Speak, the note is being written..."}</p>}<textarea className="inp" placeholder={lang==="fr"?"Points à aborder, contexte...":"Topics to discuss, context..."} value={mtgNotes} onChange={e=>setMtgNotes(e.target.value)} rows={3} style={{borderColor:mtgDictating?"rgba(239,68,68,.3)":"var(--b)"}}/></div>
    ...
    {showRec&&<div className="bsbg" onClick={()=>{if(!isRec&&!recProcessing){setShowRec(false);setTranscript("")}}}><div className="bsm" onClick={e=>e.stopPropagation()}>
      ...
      <div style={{background:"var(--bg3)",border:"1px solid var(--b)",borderRadius:"var(--rs)",padding:"16px",minHeight:120,maxHeight:280,overflowY:"auto",marginBottom:20}}>
        ...
        :isRec?<>{transcript?<p style={{fontSize:13,color:"var(--t2)",lineHeight:1.65,whiteSpace:"pre-wrap"}}>{transcript}</p>:<p style={{fontSize:13,color:"var(--t5)",fontStyle:"italic"}}>{lang==="fr"?"En écoute…":"Listening…"}</p>}<div style={{display:"flex",alignItems:"center",gap:8,marginTop:12}}><div style={{width:8,height:8,borderRadius:"50%",background:"#EF4444",animation:"pd 1s ease-in-out infinite"}}/><span style={{fontSize:11,color:"#991B1B"}}>{t("rec_recording")}</span></div></>
