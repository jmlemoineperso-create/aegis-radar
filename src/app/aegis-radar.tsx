...
function Logo({name,sz=24,fallback}){const d=DOMAINS[name];const[src,setSrc]=useState(d?`https://logo.clearbit.com/${d}`:null);const[failed,setFailed]=useState(0);if(!d||failed>=2)return <span className="mono" style={{width:sz,height:sz,fontSize:sz*.42}}>{fallback||(name||"?").charAt(0)}</span>;return <img src={src} alt="" style={{width:sz,height:sz,borderRadius:sz*.2,objectFit:"contain",background:"white",padding:1,flexShrink:0}} onError={()=>{if(failed===0){setSrc(`https://www.google.com/s2/favicons?domain=${d}&sz=${Math.min(sz*2,128)}`);setFailed(1)}else setFailed(2)}}/>}
    ...
    <div style={{paddingBottom:100}}>
      ...
      <div style={{padding:"24px 20px"}}>
        ...
        <div className="fi" style={{marginBottom:28}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}><Logo name={co.name} sz={44} fallback={co.logo}/><div><h1 className="fd" style={{fontSize:24,fontWeight:700,color:"var(--t1)",lineHeight:1.2}}>{co.name}</h1><p style={{fontSize:13,color:"var(--t4)",marginTop:3}}>{tx(co.sector,lang)}</p></div></div>
      ...
      <div style={{padding:"18px 20px"}}>
        ...
        {sorted.map((c,i)=>{const sc=getSigs(c.id).length;const nc=getNotes(c.id).length;const lines=getLinesAll(getSigs(c.id));return (
          <div key={c.id} className={`fi fi${Math.min(i+1,5)}`} style={{padding:"10px 16px",borderBottom:"1px solid var(--b)",background:i%2===0?"transparent":"rgba(13,20,36,.4)",display:"grid",gridTemplateColumns:"32px 1fr 80px 50px 50px 36px 22px",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>setSC(c.id)}>
              ...
              <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"flex-start"}}>{lines.length>0?lines.slice(0,2).map(l=><span key={l} style={{fontSize:7,padding:"1px 5px",borderRadius:5,background:"rgba(96,165,250,.1)",color:"#93C5FD",whiteSpace:"nowrap"}}>{lineLbl(l,lang)}</span>):<span style={{fontSize:7,color:"var(--t5)"}}>—</span>}</div>
              <div style={{textAlign:"center"}}><span style={{fontSize:11,color:sc>0?"#60A5FA":"var(--t5)",fontWeight:sc>0?600:400}}>{sc}</span><p style={{fontSize:7,color:"var(--t5)",marginTop:1}}>{t("signals_lc")}</p></div>
              ...
              <button style={{width:20,height:20,borderRadius:5,background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.12)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={e=>{e.stopPropagation();togW(c.id)}}><I.x style={{width:9,height:9,color:"#F87171"}}/></button>
          </div>)})}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingRight:24}}>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  ...
                  <span className="ftag" style={{background:mtg.type==="broker"?"rgba(201,168,76,.15)":mtg.type==="autre"?"rgba(139,92,246,.15)":"rgba(96,165,250,.15)",color:mtg.type==="broker"?"var(--gold)":mtg.type==="autre"?"#C4B5FD":"#93C5FD"}}>{mtg.type==="broker"?t("meeting_broker"):mtg.type==="rm"?t("meeting_rm"):mtg.type==="autre"?t("meeting_other"):t("meeting_internal")}</span>
                </div>
                ...
                {mtg.contact&&mtg.contact.name&&<div style={{marginTop:6,padding:"6px 10px",background:"rgba(139,92,246,.06)",borderRadius:6,border:"1px solid rgba(139,92,246,.12)"}}>
                  <p style={{fontSize:11,fontWeight:600,color:"#C4B5FD"}}>{mtg.contact.name}{mtg.contact.role&&<span style={{fontWeight:400,color:"var(--t4)"}}> — {mtg.contact.role}</span>}</p>
