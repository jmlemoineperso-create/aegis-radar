...
function Logo({name,sz=24,fallback}){const d=DOMAINS[name];const[src,setSrc]=useState(d?`https://logo.clearbit.com/${d}`:null);const[failed,setFailed]=useState(0);if(!d||failed>=3)return <span className="mono" style={{width:sz,height:sz,fontSize:sz*.42}}>{fallback||(name||"?").charAt(0)}</span>;return <img src={src} alt="" style={{width:sz,height:sz,borderRadius:sz*.18,objectFit:"contain",background:"white",padding:1,flexShrink:0}} onError={()=>{if(failed===0){setSrc(`https://www.google.com/s2/favicons?domain=${d}&sz=${Math.max(sz*2,64)}`);setFailed(1)}else if(failed===1){setSrc(`https://icon.horse/icon/${d}`);setFailed(2)}else setFailed(3)}}/>}
