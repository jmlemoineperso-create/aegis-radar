...
const T={
  ...
  dashboard_title:{en:"Financial Lines Intelligence",fr:"Financial Lines Intelligence"},
  ...
  dedication:{en:"© Designed & built by AIG",fr:"© Conçu et développé par AIG"},
...
const LVL_C={critical:"#DC2626",high:"#F59E0B",medium:"#3B82F6",low:"#10B981"};
...
const NOTE_C={observation:{c:"#92400E",bg:"rgba(245,158,11,.06)"},hypothesis:{c:"#5B21B6",bg:"rgba(139,92,246,.06)"},action:{c:"#166534",bg:"rgba(16,185,129,.06)"},question:{c:"#1E40AF",bg:"rgba(59,130,246,.06)"},decision:{c:"#9D174D",bg:"rgba(244,114,182,.06)"}};
...
const factLbl=(f,t)=>({verified:{l:t("verified"),c:"#166534",bg:"rgba(16,185,129,.1)"},likely:{l:t("likely"),c:"#92400E",bg:"rgba(245,158,11,.1)"},hypothesis:{l:t("hypothesis_fact"),c:"#5B21B6",bg:"rgba(139,92,246,.1)"},needs_review:{l:t("needs_review"),c:"#9CA3AF",bg:"rgba(156,163,175,.1)"}}[f]||{l:f,c:"#9CA3AF",bg:"rgba(156,163,175,.1)"});
...
const sBg=s=>s>=80?"#FEF2F2":s>=60?"#FFFBEB":s>=40?"#EFF6FF":"#F0FDF4";
const sT=s=>s>=80?"#991B1B":s>=60?"#92400E":s>=40?"#1E40AF":"#166534";
const tI=t=>t==="rising"?"↑":t==="declining"?"↓":"→";

// ── CSS ──
const css=`:root{--navy:#002B5C;--navy-dark:#001E42;--navy-light:#003D80;--accent:#0072CE;--accent-light:#3D9BE0;--accent-bg:#E8F2FC;--n-white:#FFFFFF;--n-50:#FAFBFC;--n-100:#F3F5F7;--n-200:#E2E6EB;--n-300:#CDD3DA;--n-400:#A8B1BD;--n-500:#7D8A9A;--n-600:#5C6B7D;--n-700:#3D4E63;--n-800:#263348;--n-900:#141E2B;--r:8px;--rs:6px;--mw:480px}
*{margin:0;padding:0;box-sizing:border-box}body,#root{font-family:'Segoe UI',-apple-system,BlinkMacSystemFont,'Helvetica Neue',Arial,sans-serif;background:var(--n-100);color:var(--n-700);-webkit-font-smoothing:antialiased;min-height:100vh}
.app{max-width:var(--mw);margin:0 auto;min-height:100vh;background:var(--n-100);position:relative;overflow-x:hidden}
::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:var(--n-300);border-radius:3px}
...
.pd{width:6px;height:6px;border-radius:50%;background:#16A34A;animation:pd 2.5s ease-in-out infinite}
.card{background:var(--n-white);border:1px solid var(--n-200);border-radius:var(--r);transition:border-color .2s}.card:hover{border-color:var(--n-300);box-shadow:0 1px 4px rgba(0,43,92,.06)}
.card-el{background:var(--n-white);border:1px solid var(--n-200);border-radius:var(--r)}
.cs{background:var(--n-50);border:1px solid var(--n-200);border-radius:var(--rs);padding:14px 16px}
...
.bp{padding:11px 22px;border-radius:var(--rs);font-size:13px;font-weight:600;color:#fff;background:var(--navy)}.bp:disabled{opacity:.4;cursor:not-allowed}.bp:hover:not(:disabled){background:var(--navy-light)}
.bg{padding:4px 0;background:transparent;color:var(--n-600);font-size:13px;font-weight:500}
.bi{width:36px;height:36px;padding:0;border-radius:var(--rs);display:flex;align-items:center;justify-content:center;background:var(--n-white);border:1px solid var(--n-200);color:var(--n-600);cursor:pointer;transition:all .2s}.bi:hover{background:var(--n-50);color:var(--navy);border-color:var(--n-300)}
.chip{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:20px;font-size:12px;font-weight:500;background:var(--n-white);color:var(--n-600);border:1px solid var(--n-200);cursor:pointer;transition:all .2s;white-space:nowrap;flex-shrink:0}.chip:hover{border-color:var(--n-300)}.chip.on{background:rgba(0,114,206,.06);color:var(--navy);border-color:rgba(0,114,206,.25)}
...
.inp{width:100%;padding:11px 16px;border-radius:var(--rs);background:var(--n-white);border:1px solid var(--n-200);color:var(--n-800);font-family:inherit;font-size:14px;outline:none;transition:all .2s}.inp:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(0,114,206,.08)}.inp::placeholder{color:var(--n-400)}textarea.inp{resize:vertical;min-height:88px;line-height:1.6}
.lbl{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.1em}
.dv{height:1px;background:var(--n-200);margin:20px 0}
.aline{height:2px;background:linear-gradient(90deg,var(--navy),rgba(0,43,92,.3),transparent);border-radius:1px}
.hdr{position:sticky;top:0;z-index:50;background:var(--navy);padding:14px 20px}
.tbar{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:var(--mw);display:flex;background:rgba(255,255,255,.97);backdrop-filter:blur(12px);border-top:1px solid var(--n-200);z-index:100;padding:0 0 env(safe-area-inset-bottom,6px)}
.tbar button{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;padding:10px 0 6px;color:var(--n-400);background:none;border:none;cursor:pointer;transition:color .2s;font-family:inherit}.tbar button.on{color:var(--navy)}.tbar span{font-size:9px;font-weight:600;letter-spacing:.06em;text-transform:uppercase}
...
.bsm{width:100%;max-width:var(--mw);max-height:92vh;overflow-y:auto;background:var(--n-white);border-top:1px solid var(--n-200);border-radius:14px 14px 0 0;padding:8px 22px 28px;animation:su .3s cubic-bezier(.22,1,.36,1)}
.toast{position:fixed;bottom:76px;left:50%;transform:translateX(-50%);background:var(--navy);color:#fff;padding:10px 22px;border-radius:var(--r);font-size:13px;font-weight:500;box-shadow:0 4px 16px rgba(0,43,92,.18);z-index:300;animation:fi .2s ease}
.mono{width:32px;height:32px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-weight:600;font-size:14px;color:#fff;background:var(--navy);flex-shrink:0}
...
.prio-primary{border-left:3px solid var(--navy)}.prio-secondary{border-left:3px solid var(--accent)}.prio-watch{border-left:3px solid var(--n-300)}
.lang-sw{display:flex;border-radius:var(--rs);overflow:hidden;border:1px solid var(--n-200)}.lang-sw button{flex:1;padding:8px 16px;font-size:13px;font-weight:500;border:none;cursor:pointer;transition:all .2s;font-family:inherit;background:var(--n-50);color:var(--n-600)}.lang-sw button.on{background:rgba(0,114,206,.06);color:var(--navy)}
...
@media(min-width:768px){
  ...
  .bsm{border-radius:14px 14px 0 0;max-width:600px;padding:12px 32px 32px}
...
@media(min-width:1024px){
  ...
  .bsm{max-width:640px;border-radius:12px;margin-bottom:40px}
  .bsbg{align-items:center}
  .app{border-left:1px solid var(--n-200);border-right:1px solid var(--n-200)}
}`;
...
function Logo({name,sz=24,fallback}){const d=DOMAINS[name];const[src,setSrc]=useState(d?`https://www.google.com/s2/favicons?domain=${d}&sz=${Math.max(sz*2,64)}`:null);const[failed,setFailed]=useState(0);if(!d||failed>=2)return <span className="mono" style={{width:sz,height:sz,fontSize:sz*.42}}>{fallback||(name||"?").charAt(0)}</span>;return <img src={src} alt="" style={{width:sz,height:sz,borderRadius:sz*.18,objectFit:"contain",background:"white",padding:1,flexShrink:0}} onError={()=>{if(failed===0){setSrc(`https://logo.clearbit.com/${d}`);setFailed(1)}else setFailed(2)}}/>}

function SR({s,sz=44,sw=3}){const r=(sz-sw*2)/2,c=2*Math.PI*r,o=c-(s/100)*c,col=sC(s);return (<div className="sr" style={{width:sz,height:sz}}><svg width={sz} height={sz}><circle cx={sz/2} cy={sz/2} r={r} fill="none" stroke="var(--n-200)" strokeWidth={sw}/><circle cx={sz/2} cy={sz/2} r={r} fill="none" stroke={col} strokeWidth={sw} strokeDasharray={c} strokeDashoffset={o} strokeLinecap="round" style={{transition:"stroke-dashoffset .6s cubic-bezier(.22,1,.36,1)"}}/></svg><span className="sr-v" style={{color:col,fontSize:sz*.29}}>{s}</span></div>)}
...
function RiskChart({data,h=100}){
  ...
  return (<svg viewBox={`0 0 ${w} ${h}`} style={{width:"100%",height:h}}>
    ...
    {[25,50,75].filter(v=>v>=mn&&v<=mx).map(v=>{const y=pad+(1-(v-mn)/(mx-mn||1))*gh;return (<g key={v}><line x1={pad} y1={y} x2={w-pad} y2={y} stroke="var(--n-200)" strokeWidth="0.5" strokeDasharray="3,3"/><text x={pad-4} y={y+3} fill="var(--n-400)" fontSize="8" textAnchor="end">{v}</text></g>)})}
    ...
    {data.length>1&&<><text x={pad} y={h-4} fill="var(--n-400)" fontSize="8">{data[0].date.slice(5)}</text><text x={w-pad} y={h-4} fill="var(--n-400)" fontSize="8" textAnchor="end">{data[data.length-1].date.slice(5)}</text></>}
  </svg>);
}
...
function App(){
  ...
  const mtgLabel=(mtg)=>{const d=new Date(mtg.date);const now=new Date();const diff=Math.ceil((d-now)/(86400000));if(diff===0)return{l:t("today"),c:"#DC2626"};if(diff===1)return{l:t("tomorrow"),c:"#D97706"};if(diff>0)return{l:`${diff} ${t("days_left")}`,c:"#0072CE"};return{l:fD(mtg.date),c:"var(--n-400)"}};
    ...
    try{
      ...
      if(data.signals&&data.signals.length>0){
        ...
        if(crits.length>0&&typeof Notification!=="undefined"&&Notification.permission==="granted"){
          ...
          new Notification("AIG FL Intelligence",{body:`${lang==="fr"?"Signal critique":"Critical signal"}: ${co?.name||""} — ${tx(crits[0].title,lang)}`,icon:"/icon-192.png",badge:"/icon-192.png",tag:"signalis-"+crits[0].id});
          if(crits.length>1)new Notification("AIG FL Intelligence",{body:`${lang==="fr"?`+ ${crits.length-1} autre(s) signal(aux) critique(s)`:`+ ${crits.length-1} more critical signal(s)`}`,icon:"/icon-192.png",tag:"signalis-batch"});
        }
        setLastRefresh(new Date().toISOString());
  ...
  if(step==="login")return (
    <div className="fi" style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"48px 28px",background:"radial-gradient(ellipse at 50% -10%,rgba(0,43,92,.07) 0%,transparent 55%),var(--n-100)"}}>
      <div style={{textAlign:"center",marginBottom:48}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12,marginBottom:12}}><I.radar/><span style={{fontSize:30,fontWeight:700,color:"var(--n-800)",letterSpacing:".02em",color:"var(--navy)"}}>AIG Financial Lines Intelligence</span></div>
        <p className="lbl" style={{color:"var(--n-500)",letterSpacing:".14em",fontSize:9}}>Financial Lines Intelligence</p>
      ...
      <div style={{width:"100%",maxWidth:320}}>
        <h2 style={{fontSize:20,fontWeight:600,color:"var(--n-800)",textAlign:"center",marginBottom:6}}>{t("onboarding_title")}</h2>
        <p style={{fontSize:13,color:"var(--n-600)",textAlign:"center",lineHeight:1.55,marginBottom:32}}>{t("onboarding_sub")}</p>
        <div style={{marginBottom:18}}><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:8}}>{t("email")}</label><input className="inp" type="email" placeholder="asprevel@gmail.com" value={loginEm} onChange={e=>{setLoginEm(e.target.value);setLoginErr(false)}} style={loginErr?{borderColor:"#DC2626"}:{}}/></div>
        <div style={{marginBottom:28}}><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:8}}>{t("password")}</label><input className="inp" type="password" placeholder="••••••••" value={loginPw} onChange={e=>{setLoginPw(e.target.value);setLoginErr(false)}} onKeyDown={e=>e.key==="Enter"&&tryLogin()} style={loginErr?{borderColor:"#DC2626"}:{}}/>{loginErr&&<p style={{fontSize:12,color:"#991B1B",marginTop:8}}>{t("login_err")}</p>}</div>
        <button className="btn bp" style={{width:"100%",height:46}} onClick={tryLogin}><I.lock/>{t("onboarding_cta")}</button>
        <p style={{textAlign:"center",marginTop:24,fontSize:10,color:"var(--n-400)"}}>© 2026 AIG FL Intelligence — AIG</p>
      </div>
    </div>
  );
  ...
  if(step==="select"){const selCount=cos.filter(c=>c.prio).length;return (
    <div className="fi" style={{minHeight:"100vh",padding:"40px 20px 100px",background:"var(--n-100)"}}>
      <div style={{textAlign:"center",marginBottom:32}}><I.radar style={{margin:"0 auto 12px",display:"block",color:"var(--navy)"}}/><h2 style={{fontSize:22,fontWeight:600,color:"var(--n-800)",marginBottom:8}}>{t("select_companies")}</h2><p style={{fontSize:13,color:"var(--n-600)",lineHeight:1.55}}>{t("select_companies_sub")}</p></div>
      <div className="co-grid" style={{marginBottom:24}}>{cos.map(c=>{const on=!!c.prio;return (<button key={c.id} className="card" style={{padding:"14px 18px",width:"100%",textAlign:"left",cursor:"pointer",borderColor:on?"rgba(0,43,92,.3)":"var(--n-200)",background:on?"rgba(0,114,206,.06)":"var(--bg2)"}} onClick={()=>togW(c.id,"watch")}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:22,height:22,borderRadius:6,border:`2px solid ${on?"var(--navy)":"var(--n-300)"}`,background:on?"var(--navy)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{on&&<span style={{color:"var(--n-100)",fontSize:14,fontWeight:700}}>✓</span>}</div><Logo name={c.name} sz={28} fallback={c.logo}/><div style={{flex:1,minWidth:0}}><h4 style={{fontSize:13,fontWeight:600,color:on?"var(--n-800)":"var(--n-700)"}}>{c.name}</h4><p style={{fontSize:11,color:"var(--n-500)",marginTop:1}}>{tx(c.sector,lang)}</p></div><SR s={c.risk} sz={34} sw={2}/></div></button>)})}</div>
      <div className="tbar" style={{flexDirection:"column",gap:0,padding:"16px 20px"}}><button className="btn bp" style={{width:"100%",height:46}} onClick={()=>setStep("app")} disabled={selCount===0}>{t("continue_btn")} {selCount>0&&`(${selCount} ${t("selected")})`}</button><button className="btn" style={{width:"100%",marginTop:8,color:"var(--n-500)",fontSize:12,background:"none"}} onClick={()=>setStep("app")}>{t("skip")}</button></div>
    </div>);}
  ...
  const SigCard=({s,d=0})=>{const cat=getCat(s.cat,lang);const co=cos.find(c=>c.id===s.cid)||cos.find(c=>s.company&&(c.name.toLowerCase()===s.company.toLowerCase()||s.company.toLowerCase().includes(c.name.toLowerCase().split(" ")[0])));const imps=getImpsAll(s.id);return (
    <div className={`card fi ${d>0?`fi${Math.min(d,5)}`:""}`} style={{padding:"16px 18px",cursor:"pointer"}} onClick={()=>setSS(s)}>
      ...
      <h3 style={{fontSize:14,fontWeight:600,color:"var(--n-800)",lineHeight:1.4,marginBottom:8}}>{tx(s.title,lang)}</h3>
      <p style={{fontSize:12,color:"var(--n-600)",lineHeight:1.45,marginBottom:12,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{tx(s.sum,lang)}</p>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:6}}>{co&&<Logo name={co.name} sz={20} fallback={co.logo}/>}<span style={{fontSize:11,fontWeight:500,color:"var(--navy)"}}>{co?.name}</span><span style={{fontSize:10,color:"var(--n-400)"}}>·</span><span style={{fontSize:10,color:"var(--n-400)"}}>{tx(s.src,lang)}</span></div><span style={{fontSize:10,color:"var(--n-400)",display:"flex",alignItems:"center",gap:4}}><I.clock/>{fD(s.at)}</span></div>
      {imps.length>0&&<div style={{display:"flex",gap:5,marginTop:10,flexWrap:"wrap"}}>{imps.map((im,idx)=><span key={`${im.line}-${idx}`} style={{fontSize:10,padding:"2px 8px",borderRadius:12,background:LVL_BG[im.lvl]||"rgba(59,130,246,.1)",color:LVL_T[im.lvl]||"#1E40AF",border:`1px solid ${(LVL_C[im.lvl]||"#3B82F6")}22`}}>{lineLbl(im.line,lang)} · {im.lvl||"—"}</span>)}</div>}
    </div>)};
  ...
  const SigDet=({s,onClose})=>{if(!s)return null;const cat=getCat(s.cat,lang);const co=cos.find(c=>c.id===s.cid)||cos.find(c=>s.company&&(c.name.toLowerCase()===s.company.toLowerCase()||s.company.toLowerCase().includes(c.name.toLowerCase().split(" ")[0])));const f=factLbl(s.fact,t);const imps=getImpsAll(s.id);return (
    <div className="bsbg" onClick={onClose}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      ...
      <h2 style={{fontSize:20,fontWeight:600,color:"var(--n-800)",lineHeight:1.35,marginBottom:14}}>{tx(s.title,lang)}</h2>
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:16,flexWrap:"wrap"}}><span className="badge" style={{background:sBg(s.imp),color:sT(s.imp)}}>{scoreLbl(s.imp,t)} · {s.imp}</span><span className="ftag" style={{background:f.bg,color:f.c}}>{f.l}</span><span style={{fontSize:11,color:"var(--n-500)",display:"flex",alignItems:"center",gap:4}}><I.clock/>{fD(s.at)}</span></div>
      {co&&<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--bg3)",borderRadius:"var(--rs)",marginBottom:18,border:"1px solid var(--n-200)"}}><Logo name={co.name} sz={28} fallback={co.logo}/><span style={{fontSize:13,fontWeight:600,color:"var(--n-800)"}}>{co.name}</span><span style={{fontSize:11,color:"var(--n-500)",marginLeft:10}}>{tx(co.sector,lang)}</span></div>}
      <div className="dv"/>
      <div style={{marginBottom:22}}><h4 className="lbl" style={{color:"var(--n-600)",marginBottom:10}}>{t("what_happened")}</h4><p style={{fontSize:13,color:"var(--n-700)",lineHeight:1.65}}>{tx(s.sum,lang)}</p></div>
      <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:22}}><span style={{fontSize:11,color:"var(--n-500)"}}>{t("source_label")}</span>{srcUrl(s.src)?<a href={srcUrl(s.src)} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:"var(--navy)",fontWeight:500,textDecoration:"none",display:"flex",alignItems:"center",gap:4}}>{tx(s.src,lang)}<I.ext/></a>:<span style={{fontSize:11,color:"var(--navy)",fontWeight:500}}>{tx(s.src,lang)}</span>}</div>
      <div className="dv"/>
      <h4 className="lbl" style={{color:"var(--n-600)",marginBottom:12}}>{t("lines_impacted")}</h4>
      {imps.map((im,idx)=><div key={`${im.line}-${idx}`} style={{marginBottom:idx<imps.length-1?12:22}}>
        <div style={{padding:"16px 18px",background:LVL_BG[im.lvl]||"rgba(59,130,246,.1)",borderRadius:"var(--rs)",border:`1px solid ${(LVL_C[im.lvl]||"#3B82F6")}18`,borderLeft:`3px solid ${LVL_C[im.lvl]||"#3B82F6"}`}}>
          ...
          <div style={{marginBottom:12}}><p className="lbl" style={{color:"var(--n-500)",marginBottom:6,fontSize:9}}>{t("why_matters")}</p><p style={{fontSize:13,color:"var(--n-800)",lineHeight:1.6}}>{tx(im.why,lang)}</p></div>
          {im.angle&&<div style={{marginBottom:12}}><p className="lbl" style={{color:"var(--n-500)",marginBottom:6,fontSize:9}}>{t("discussion_angle")}</p><p style={{fontSize:13,color:"var(--n-700)",lineHeight:1.55}}>{tx(im.angle,lang)}</p></div>}
          {(im.vig||[]).length>0&&<div style={{marginBottom:(im.hyp||[]).length>0?12:0}}><p className="lbl" style={{color:"var(--n-500)",marginBottom:6,fontSize:9}}>{t("points_verify")}</p>{(im.vig||[]).map((v,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:3}}><span style={{color:LVL_T[im.lvl]||"var(--n-600)",marginTop:1,flexShrink:0,fontSize:11}}>▸</span><span style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{tx(v,lang)}</span></div>)}</div>}
          {(im.hyp||[]).length>0&&<div><p className="lbl" style={{color:"var(--n-500)",marginBottom:6,fontSize:9}}>{t("hyp_check")}</p>{(im.hyp||[]).map((h,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:3}}><span className="ftag" style={{background:"rgba(139,92,246,.1)",color:"#5B21B6",flexShrink:0,marginTop:1}}>H{i+1}</span><span style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{tx(h,lang)}</span></div>)}</div>}
        </div>
      </div>)}
      <div style={{display:"flex",gap:28,marginBottom:10}}><div><h4 className="lbl" style={{color:"var(--n-500)",marginBottom:6}}>{t("importance")}</h4><SR s={s.imp||50} sz={50}/></div><div><h4 className="lbl" style={{color:"var(--n-500)",marginBottom:6}}>{t("confidence")}</h4><SR s={s.conf||50} sz={50}/></div></div>
    </div></div>)};
  ...
  const BriefSheet=({cid,onClose})=>{
    ...
    return (<div className="bsbg" onClick={onClose}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,paddingTop:8}}><h3 style={{fontSize:18,fontWeight:600,color:"var(--n-800)"}}>{t("brief_title")}</h3><button className="bi" style={{width:32,height:32}} onClick={onClose}><I.x/></button></div>
      <p style={{fontSize:12,color:"var(--n-500)",marginBottom:16}}>{t("brief_sub")}</p><div className="aline" style={{marginBottom:18}}/>

      {co&&<div className="cs" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{display:"flex",alignItems:"center",gap:12}}><Logo name={co.name} sz={36} fallback={co.logo}/><div><h4 style={{fontSize:14,fontWeight:600,color:"var(--n-800)"}}>{co.name}</h4><p style={{fontSize:11,color:"var(--n-500)",marginTop:2}}>{tx(co.sector,lang)}</p></div></div><SR s={co.risk} sz={46}/></div>}
      ...
      {lastDate&&newSince.length>0&&<div style={{padding:"12px 14px",background:"rgba(52,211,153,.06)",borderRadius:"var(--rs)",border:"1px solid rgba(52,211,153,.15)",marginBottom:18}}>
        <p style={{fontSize:11,fontWeight:700,color:"#16A34A",marginBottom:6}}>{lang==="fr"?`${newSince.length} nouveau(x) signal(aux) depuis le dernier brief`:`${newSince.length} new signal(s) since last brief`} <span style={{fontWeight:400,color:"var(--n-400)"}}>({fD(lastDate)})</span></p>
        <div style={{display:"flex",flexDirection:"column",gap:4}}>{newSince.slice(0,3).map((s,i)=><div key={i} style={{display:"flex",gap:6,alignItems:"center"}}><span className="badge" style={{background:sBg(s.imp||50),color:sT(s.imp||50),fontSize:9,padding:"1px 5px"}}>{s.imp||50}</span><span style={{fontSize:11,color:"var(--n-700)"}}>{tx(s.title,lang)}</span></div>)}{newSince.length>3&&<p style={{fontSize:10,color:"var(--n-400)",marginTop:2}}>+ {newSince.length-3} {lang==="fr"?"autre(s)":"more"}</p>}
        </div>
      </div>}
      ...
      {history.length>1&&<button style={{width:"100%",padding:"8px",marginBottom:18,background:"var(--bg3)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)",cursor:"pointer",color:"var(--n-500)",fontSize:11,fontFamily:"inherit",display:"flex",justifyContent:"space-between",alignItems:"center"}} onClick={()=>setSBH(!showBriefHist)}>
      ...
      {showBriefHist&&<div style={{marginBottom:18,maxHeight:200,overflow:"auto"}}>{history.slice(1).map((b,i)=><div key={b.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderBottom:"1px solid var(--n-200)",opacity:.7+(i===0?.3:0)}}>
        <div><p style={{fontSize:11,color:"var(--n-700)"}}>{fD(b.date)}</p><p style={{fontSize:9,color:"var(--n-400)"}}>{b.signalCount} {lang==="fr"?"signaux":"signals"} · {lang==="fr"?"Score":"Score"} {b.risk}</p></div>
      ...
      <h4 className="lbl" style={{color:"var(--navy)",marginBottom:8}}>{t("exec_summary")}</h4>
      <p style={{fontSize:13,color:"var(--n-700)",marginBottom:20,lineHeight:1.55}}>{sigs.length} {sigs.length>1?t("signals_lc"):t("signal")} · {lines.length} {t("lines_lc")} · {scoreLbl(co?.risk,t)}</p>
      <h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("key_signals")}</h4>
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>{sigs.slice(0,5).map((s,idx)=><div key={s.id||idx} style={{display:"flex",gap:10,alignItems:"flex-start"}}><span className="badge" style={{background:sBg(s.imp||50),color:sT(s.imp||50),flexShrink:0,marginTop:2}}>{s.imp||50}</span><p style={{fontSize:13,fontWeight:500,color:"var(--n-800)",lineHeight:1.4}}>{tx(s.title,lang)||s.company||"—"}</p></div>)}</div>
      <h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("fl_implications")}</h4>
      ...
      {angles.length>0&&<><h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("discussion_angles")}</h4><div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20}}>{angles.map((a,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start"}}><span style={{color:"var(--navy)",marginTop:1,flexShrink:0}}>▸</span><span style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{a}</span></div>)}</div></>}
      {/* Dossier client */}
      {dos&&(dos.broker||dos.rm||dos.renewal||dos.premium||dos.program||dos.sinistres||dos.context)&&<>
        <h4 className="lbl" style={{color:"#5B21B6",marginBottom:10}}>{lang==="fr"?"Dossier client":"Client file"}</h4>
        <div className="cs" style={{padding:"14px 16px",marginBottom:20}}>
          {(dos.broker||dos.rm)&&<div style={{display:"flex",gap:16,marginBottom:10}}>{dos.broker&&<div><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>{lang==="fr"?"Courtier":"Broker"}</p><p style={{fontSize:12,color:"var(--n-700)",fontWeight:500}}>{dos.broker}</p></div>}{dos.rm&&<div><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>Risk Manager</p><p style={{fontSize:12,color:"var(--n-700)",fontWeight:500}}>{dos.rm}</p></div>}</div>}
          {(dos.renewal||dos.premium)&&<div style={{display:"flex",gap:16,marginBottom:10}}>{dos.renewal&&<div><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>{lang==="fr"?"Renouvellement":"Renewal"}</p><p style={{fontSize:12,color:"#D97706",fontWeight:600}}>{new Date(dos.renewal).toLocaleDateString(lang==="fr"?"fr-FR":"en-GB",{day:"numeric",month:"long",year:"numeric"})}</p></div>}{dos.premium&&<div><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>{lang==="fr"?"Prime":"Premium"}</p><p style={{fontSize:12,color:"var(--n-700)",fontWeight:500}}>{dos.premium}</p></div>}</div>}
          {dos.program&&<div style={{marginBottom:8}}><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>{lang==="fr"?"Programme FL":"FL Programme"}</p><p style={{fontSize:11,color:"var(--n-700)",lineHeight:1.5}}>{dos.program}</p></div>}
          {dos.sinistres&&<div style={{marginBottom:8}}><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>{lang==="fr"?"Sinistralité":"Claims"}</p><p style={{fontSize:11,color:"var(--n-700)",lineHeight:1.5}}>{dos.sinistres}</p></div>}
          {dos.context&&<div><p className="lbl" style={{color:"var(--n-400)",fontSize:8,marginBottom:2}}>{lang==="fr"?"Contexte":"Context"}</p><p style={{fontSize:11,color:"var(--n-700)",lineHeight:1.5}}>{dos.context}</p></div>}
        </div>
      </>}
      {!dos&&<div style={{padding:"10px 14px",background:"rgba(139,92,246,.04)",borderRadius:"var(--rs)",border:"1px dashed rgba(139,92,246,.15)",marginBottom:20,textAlign:"center"}}><p style={{fontSize:11,color:"var(--n-400)"}}>{lang==="fr"?"Ajoutez un dossier client pour enrichir ce brief":"Add a client file to enrich this brief"}</p><button className="btn" style={{padding:"4px 12px",fontSize:10,color:"#5B21B6",background:"rgba(139,92,246,.1)",border:"1px solid rgba(139,92,246,.2)",borderRadius:6,marginTop:6}} onClick={()=>{onClose();setTimeout(()=>openDossier(cid),300)}}>+ {lang==="fr"?"Créer le dossier":"Create file"}</button></div>}
      <h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("interlocutors")}</h4>
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        <div className="cs" style={{flex:1,padding:"12px 14px"}}><p className="lbl" style={{color:"var(--n-400)",fontSize:9,marginBottom:4}}>{t("broker")}</p><p style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{lang==="fr"?"Partager les signaux clés et discuter du positionnement du programme FL lors du prochain comité de renouvellement.":"Share key signals and discuss FL programme positioning at next renewal committee."}</p></div>
        <div className="cs" style={{flex:1,padding:"12px 14px"}}><p className="lbl" style={{color:"var(--n-400)",fontSize:9,marginBottom:4}}>{t("risk_manager")}</p><p style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{lang==="fr"?"Valider la perception du risque et confirmer les mesures de prévention mises en place par l'entreprise.":"Validate risk perception and confirm prevention measures implemented by the company."}</p></div>
      </div>
      {questions.length>0&&<><h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("questions_to_ask")}</h4><div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20}}>{questions.map((q,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start"}}><span style={{color:"#5B21B6",marginTop:1,flexShrink:0}}>?</span><span style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{q}</span></div>)}</div></>}
      <h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("next_steps")}</h4>
      <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20}}>{actions.length>0?actions.map((a,i)=><div key={i} style={{display:"flex",gap:8,alignItems:"flex-start"}}><span style={{color:"#16A34A",marginTop:1,flexShrink:0}}>→</span><span style={{fontSize:12,color:"var(--n-700)",lineHeight:1.5}}>{a}</span></div>):<p style={{fontSize:12,color:"var(--n-500)"}}>{t("to_be_defined")}</p>}</div>
      ...
      <div style={{padding:"16px 18px",background:"linear-gradient(135deg,rgba(0,43,92,.04),rgba(139,92,246,.04))",borderRadius:"var(--rs)",border:"1px dashed rgba(0,43,92,.2)",marginBottom:20}}>
        <p style={{fontSize:10,fontWeight:700,color:"var(--navy)",textTransform:"uppercase",letterSpacing:".08em",marginBottom:10}}>{lang==="fr"?"AIG FL Intelligence pourrait aller plus loin":"AIG FL Intelligence could go further"}</p>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[
            ...
            <div><p style={{fontSize:11,fontWeight:600,color:"var(--n-700)"}}>{v.l}</p><p style={{fontSize:10,color:"var(--n-500)",lineHeight:1.4}}>{v.d}</p></div>
          </div>)}
        </div>
        <p style={{fontSize:9,color:"var(--n-400)",marginTop:10,fontStyle:"italic",textAlign:"center"}}>{lang==="fr"?"Ces fonctionnalités nécessitent une connexion aux outils de l'entreprise.":"These features require connection to enterprise tools."}</p>
      </div>

      <div style={{display:"flex",gap:8,marginTop:4}}>
        <button className="btn" style={{flex:1,height:44,background:"var(--bg3)",color:"var(--n-700)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)",fontSize:12,fontWeight:600}} onClick={()=>{saveBrief(cid,txt);navigator.clipboard?.writeText(txt);setCopied(true);showT(t("copied_clipboard"));setTimeout(()=>setCopied(false),1500)}}>{copied?<><I.check/>{t("copied")}</>:<><I.copy/>{t("copy_brief")}</>}</button>
        <button className="btn" style={{flex:1,height:44,background:"var(--bg3)",color:"var(--n-700)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)",fontSize:12,fontWeight:600}} onClick={()=>{
          ...
          w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>AIG FL Intelligence Brief — ${co?.name||""}</title><style>body{font-family:'Segoe UI',Arial,sans-serif;max-width:700px;margin:40px auto;padding:0 24px;color:#1f2937}h1{font-weight:600;font-size:24px;color:#111827;margin-bottom:4px}h2{font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#002B5C;margin:24px 0 10px;font-weight:700;border-bottom:2px solid #E2E6EB;padding-bottom:4px}table{width:100%;border-collapse:collapse}.header{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #002B5C;padding-bottom:16px;margin-bottom:24px}.logo{font-weight:700;font-size:20px;font-weight:700;color:#002B5C}.date{font-size:12px;color:#6b7280}.company{background:#f8f9fc;border:1px solid #e5e7eb;border-radius:10px;padding:16px;margin-bottom:20px;display:flex;align-items:center;gap:14px}.mono{width:40px;height:40px;border-radius:8px;background:linear-gradient(135deg,#1e3a5f,#0f2440);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-weight:700;font-size:16px}.score{width:44px;height:44px;border-radius:50%;border:3px solid ${sC(co?.risk||50)};display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:${sC(co?.risk||50)}}.contacts{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:12px 0}.contact{background:#f8f9fc;border:1px solid #e5e7eb;border-radius:8px;padding:12px}.contact h4{font-size:10px;text-transform:uppercase;letter-spacing:0.08em;color:#6b7280;margin-bottom:6px}.contact p{font-size:12px;color:#374151;line-height:1.5}@media print{body{margin:20px}@page{margin:1.5cm}}</style></head><body>
          <div class="header"><div><span class="logo">AIG FL Intelligence</span><br><span style="font-size:10px;text-transform:uppercase;letter-spacing:0.12em;color:#9ca3af">Financial Lines Intelligence</span></div><span class="date">${new Date().toLocaleDateString(lang==="fr"?"fr-FR":"en-GB",{day:"numeric",month:"long",year:"numeric"})}</span></div>
          ...
          <div style="margin-top:40px;padding-top:16px;border-top:2px solid #E2E6EB;display:flex;justify-content:space-between;align-items:center"><span style="font-size:10px;color:#9ca3af">© 2026 AIG — Financial Lines Intelligence</span><span style="font-size:10px;color:#9ca3af">${new Date().toLocaleDateString(lang==="fr"?"fr-FR":"en-GB")}</span></div>
          </body></html>`);
  ...
  const CompPage=({cid})=>{const co=cos.find(c=>c.id===cid);const sigs=getSigs(cid);const cn=getNotes(cid);const lines=getLinesAll(sigs);if(!co)return null;return (
    <div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><button className="btn bg" style={{gap:4}} onClick={()=>setSC(null)}><I.chL/>{t("back")}</button><div style={{display:"flex",gap:8}}><button className="bi" style={{width:34,height:34}} onClick={()=>togW(co.id)}>{co.prio?<I.star style={{color:"var(--navy)"}}/>:<I.starO/>}</button><button className="btn bp" style={{padding:"6px 14px",fontSize:12}} onClick={()=>{setBC(co.id);setSB(true);setCopied(false)}}><I.brief style={{width:14,height:14}}/>{t("generate_brief")}</button></div></div></div>
      <div style={{padding:"24px 20px"}}>
        <p className="lbl" style={{color:"var(--n-500)",marginBottom:12}}>{t("company_overview")}</p>
        <div className="fi" style={{marginBottom:28}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}><Logo name={co.name} sz={44} fallback={co.logo}/><div><h1 style={{fontSize:24,fontWeight:700,color:"var(--n-800)",lineHeight:1.2}}>{co.name}</h1><p style={{fontSize:13,color:"var(--n-500)",marginTop:3}}>{tx(co.sector,lang)}</p></div></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
            {[{l:t("headquarters"),v:co.hq},{l:t("market_cap"),v:co.cap},{l:t("employees"),v:co.emp}].map(x=><div key={x.l} className="cs" style={{padding:"10px 12px"}}><p className="lbl" style={{color:"var(--n-400)",fontSize:9,marginBottom:4}}>{x.l}</p><p style={{fontSize:12,color:"var(--n-700)",fontWeight:500}}>{x.v||"—"}</p></div>)}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12,marginTop:12,padding:"12px 16px",background:"var(--bg3)",borderRadius:"var(--rs)",border:"1px solid var(--n-200)"}}><SR s={co.risk} sz={42}/><div><p className="lbl" style={{color:"var(--n-500)",fontSize:9}}>{t("risk_score")}</p><p style={{fontSize:13,fontWeight:600,color:sC(co.risk)}}>{scoreLbl(co.risk,t)} · {tI(co.trend)} {co.trend}</p></div></div>
          {riskHistory[co.id]&&riskHistory[co.id].length>=2?<div style={{marginTop:12,padding:"14px 16px",background:"var(--bg3)",borderRadius:"var(--rs)",border:"1px solid var(--n-200)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><p className="lbl" style={{color:"var(--n-500)",fontSize:9}}>{t("risk_history")}</p><span style={{fontSize:9,color:"var(--n-400)"}}>{t("last_30_days")}</span></div><RiskChart data={riskHistory[co.id]}/></div>:riskHistory[co.id]&&riskHistory[co.id].length===1?<p style={{fontSize:11,color:"var(--n-400)",marginTop:8,fontStyle:"italic"}}>{t("risk_no_history")}</p>:null}
        </div>
        ...
        {(()=>{const coMtgs=meetings.filter(m=>m.cid===co.id);const upcoming=coMtgs.filter(m=>new Date(m.date)>=new Date()).sort((a,b)=>new Date(a.date)-new Date(b.date));const past=coMtgs.filter(m=>new Date(m.date)<new Date()).sort((a,b)=>new Date(b.date)-new Date(a.date));if(coMtgs.length===0)return null;return (<>
          <div className="dv"/><h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}><I.calendar style={{width:12,height:12,display:"inline",marginRight:6}}/>{lang==="fr"?"Réunions":"Meetings"}</h3>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                  ...
                  <span className="ftag" style={{background:mtg.type==="broker"?"rgba(0,43,92,.15)":mtg.type==="autre"?"rgba(139,92,246,.15)":"rgba(96,165,250,.15)",color:mtg.type==="broker"?"var(--navy)":mtg.type==="autre"?"#5B21B6":"#1E40AF",fontSize:9}}>{mtg.type==="broker"?t("meeting_broker"):mtg.type==="rm"?t("meeting_rm"):mtg.type==="autre"?t("meeting_other"):t("meeting_internal")}</span>
                </div>
                <p style={{fontSize:12,color:"var(--n-700)"}}>{d.toLocaleDateString(lang==="fr"?"fr-FR":"en-GB",{weekday:"short",day:"numeric",month:"short"})} · {d.toLocaleTimeString(lang==="fr"?"fr-FR":"en-GB",{hour:"2-digit",minute:"2-digit"})}</p>
                {mtg.notes&&<p style={{fontSize:10,color:"var(--n-400)",marginTop:3,fontStyle:"italic"}}>{mtg.notes.slice(0,80)}{mtg.notes.length>80?"...":""}</p>}
              ...
              <I.chR style={{color:"var(--n-400)",flexShrink:0}}/>
            </div>
          </div>)})}
          </div>}
          {past.length>0&&<div style={{marginBottom:14}}>
            <p className="lbl" style={{color:"var(--n-400)",fontSize:9,marginBottom:8}}>{lang==="fr"?"Réunions passées":"Past meetings"}</p>
            {past.slice(0,3).map(mtg=>{const d=new Date(mtg.date);return (<div key={mtg.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderBottom:"1px solid var(--n-200)",opacity:.7,cursor:"pointer"}} onClick={()=>{setBC(co.id);setSB(true);setCopied(false)}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:10,color:"var(--n-500)"}}>{d.toLocaleDateString(lang==="fr"?"fr-FR":"en-GB",{day:"numeric",month:"short"})}</span>
                <span className="ftag" style={{background:"rgba(107,114,128,.1)",color:"var(--n-400)",fontSize:8}}>{mtg.type==="broker"?t("meeting_broker"):mtg.type==="rm"?t("meeting_rm"):mtg.type==="autre"?t("meeting_other"):t("meeting_internal")}</span>
                {mtg.contact&&mtg.contact.name&&<span style={{fontSize:9,color:"var(--n-400)"}}>· {mtg.contact.name}</span>}
              </div>
              <I.chR style={{width:10,height:10,color:"var(--n-400)"}}/>
            </div>)})}{past.length>3&&<p style={{fontSize:10,color:"var(--n-400)",marginTop:6,textAlign:"center"}}>+ {past.length-3} {lang==="fr"?"autre(s)":"more"}</p>}
          </div>}
          <button className="btn" style={{width:"100%",padding:"8px",fontSize:11,background:"var(--bg3)",color:"var(--navy)",border:"1px solid rgba(0,43,92,.2)",borderRadius:"var(--rs)",marginBottom:14}} onClick={()=>{setMtgCo(co.id);setSNM(true)}}><I.plus style={{width:12,height:12}}/>{lang==="fr"?"Planifier une réunion":"Schedule a meeting"}</button>
        </>)})()}
        <div className="dv"/><h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{t("latest_signals")}</h3>
        <div className="sig-grid" style={{marginBottom:28}}>{sigs.map((s,i)=><SigCard key={s.id} s={s} d={i+1}/>)}{sigs.length===0&&<p style={{fontSize:13,color:"var(--n-500)"}}>{t("no_signals_yet")}</p>}</div>
        <div className="dv"/><h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{t("fl_relevance")}</h3>
        {lines.length>0?<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:28}}>{lines.map(l=><span key={l} className="chip on">{lineLbl(l,lang)}</span>)}</div>:<p style={{fontSize:12,color:"var(--n-500)",marginBottom:28}}>{t("no_line_data")}</p>}
        <div className="dv"/>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><h3 className="lbl" style={{color:"var(--navy)"}}>{t("notes_title")}</h3><div style={{display:"flex",gap:6}}><button className="bi" style={{width:30,height:30,background:"rgba(239,68,68,.1)",borderColor:"rgba(239,68,68,.2)",color:"#991B1B"}} onClick={()=>{setRecCid(co.id);setShowRec(true);setTranscript("")}}><I.mic/></button><button className="bi" style={{width:30,height:30}} onClick={()=>{setNC(co.id);setSNN(true)}}><I.plus/></button></div></div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>{cn.map(n=>{const cfg=NOTE_C[n.tag]||NOTE_C.observation;return (<div key={n.id} className="card" style={{padding:"14px 16px",position:"relative"}}><button style={{position:"absolute",top:6,right:6,width:20,height:20,borderRadius:5,background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.12)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={()=>deleteN(n.id)}><I.x style={{width:10,height:10,color:"#DC2626"}}/></button><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,paddingRight:24}}><span className="ftag" style={{background:cfg.bg,color:cfg.c}}>{noteTagLbl(n.tag,t)}</span><span style={{fontSize:10,color:"var(--n-400)"}}>{fD(n.at)}</span></div><p style={{fontSize:13,color:"var(--n-700)",lineHeight:1.55}}>{tx(n.text,lang)}</p></div>)})}{cn.length===0&&<p style={{fontSize:12,color:"var(--n-500)"}}>{t("no_notes_sub")}</p>}</div>
      </div>
    </div>)};
  ...
  const render=()=>{
    ...
    if(tab==="dashboard")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{border:"1.5px solid #fff",padding:"2px 7px",color:"#fff",fontWeight:700,fontSize:14,letterSpacing:".02em"}}>AIG</span><span style={{fontSize:15,fontWeight:600,color:"rgba(255,255,255,.9)"}}>{t("dashboard_title")}</span>{autoRefresh&&<div className="pd" style={{marginLeft:4}}/>}</div><div style={{display:"flex",gap:8}}><button style={{width:34,height:34,padding:0,borderRadius:"var(--rs)",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.15)",color:"rgba(255,255,255,.8)",cursor:"pointer"}} onClick={()=>{setSSh(!showSearch);if(showSearch)setSrch("")}}>{showSearch?<I.x/>:<I.search/>}</button><button style={{width:34,height:34,padding:0,borderRadius:"var(--rs)",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.15)",color:"rgba(255,255,255,.8)",cursor:"pointer"}} onClick={()=>refreshSignals()} disabled={refreshing}><I.refresh style={{animation:refreshing?"spin 1s linear infinite":"none"}}/></button><button style={{width:34,height:34,padding:0,borderRadius:"var(--rs)",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.15)",color:"rgba(255,255,255,.8)",cursor:"pointer",position:"relative"}} onClick={()=>{setNewCount(0);try{navigator.clearAppBadge&&navigator.clearAppBadge()}catch(e){}}}><I.bell/>{newCount>0&&<div style={{position:"absolute",top:3,right:3,minWidth:16,height:16,borderRadius:8,background:"#DC2626",border:"2px solid var(--navy)",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:9,fontWeight:700,color:"white"}}>{newCount>9?"9+":newCount}</span></div>}</button></div></div>{showSearch&&<input className="inp" style={{marginTop:12,background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.15)",color:"#fff"}} placeholder={t("search_placeholder")} value={search} onChange={e=>setSrch(e.target.value)} autoFocus/>}{lastRefresh&&<p style={{fontSize:10,color:"rgba(255,255,255,.5)",marginTop:8,textAlign:"right"}}>{lang==="fr"?"Dernière mise à jour :":"Last update:"} {new Date(lastRefresh).toLocaleTimeString(lang==="fr"?"fr-FR":"en-GB",{hour:"2-digit",minute:"2-digit"})}</p>}</div>
      <div style={{padding:"18px 20px"}}><p style={{fontSize:16,fontWeight:500,color:"var(--n-800)",marginBottom:4}}>{greeting()}</p><p style={{fontSize:12,color:"var(--n-500)",marginBottom:16}}>{t("dashboard_sub")}</p>
        <div className="card-el fi" style={{padding:"16px 18px",marginBottom:22,borderLeft:"3px solid var(--navy)",cursor:"pointer"}} onClick={()=>setSD(true)}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><p className="lbl" style={{color:"var(--navy)"}}>{t("daily_digest")} — {fFull()}</p>{sbOk&&<div className="pd"/>}</div>{liveSigs.length>0?<p style={{fontSize:14,color:"var(--n-700)",lineHeight:1.45}}><strong style={{color:"var(--n-800)"}}>{liveSigs.length}</strong> {t("signal_count")} · <strong style={{color:"var(--n-800)"}}>{watched.length}</strong> {t("companies_monitored")}{digest.crit>0&&<> · <span style={{color:"#991B1B"}}>{digest.crit} {digest.crit>1?t("critical_signals"):t("critical_signal")}</span></>}</p>:<p style={{fontSize:13,color:"var(--n-500)",lineHeight:1.45}}>{t("no_data_refresh")}</p>}{lastRefresh&&<p style={{fontSize:10,color:"var(--n-400)",marginTop:6}}>{lang==="fr"?"Dernière mise à jour":"Last update"}: {new Date(lastRefresh).toLocaleTimeString(lang==="fr"?"fr-FR":"en-GB",{hour:"2-digit",minute:"2-digit"})}</p>}</div><div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>{sbOk&&<span style={{fontSize:9,color:"#16A34A",fontWeight:600}}>LIVE</span>}<I.chR style={{color:"var(--n-400)"}}/></div></div></div>
        <div className="fi fi1" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:22}}>{[{l:t("watchlist_label"),v:watched.length,c:"var(--navy)"},{l:t("active"),v:digest.total,c:"#0072CE"},{l:t("critical_lbl"),v:digest.crit,c:"#DC2626"}].map(x=>(<div key={x.l} className="card" style={{padding:"14px 12px",textAlign:"center"}}><p style={{fontSize:22,fontWeight:700,color:x.c,lineHeight:1}}>{x.v}</p><p className="lbl" style={{color:"var(--n-500)",marginTop:5,fontSize:9}}>{x.l}</p></div>))}</div>
        ...
        {watched.length>0&&<div className="fi fi2" style={{marginBottom:22}}>
          <h3 className="lbl" style={{color:"var(--navy)",marginBottom:12}}>{t("portfolio")}</h3>
          {/* Risk distribution bar */}
          {(()=>{const buckets=[{l:lang==="fr"?"Critique":"Critical",c:"#DC2626",min:75,max:100},{l:lang==="fr"?"Élevé":"High",c:"#F59E0B",min:50,max:74},{l:lang==="fr"?"Moyen":"Medium",c:"#3B82F6",min:25,max:49},{l:lang==="fr"?"Faible":"Low",c:"#10B981",min:0,max:24}];const counts=buckets.map(b=>({...b,n:watched.filter(c=>(c.risk||50)>=b.min&&(c.risk||50)<=b.max).length}));const total=watched.length||1;return (
          <div className="cs" style={{padding:"14px 16px",marginBottom:12}}>
            <p className="lbl" style={{color:"var(--n-500)",marginBottom:10,fontSize:9}}>{t("portfolio_risk")}</p>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div className="cs" style={{padding:"12px 14px"}}>
              ...
              {[...watched].sort((a,b)=>(b.risk||0)-(a.risk||0)).slice(0,5).map((c,i)=><div key={c.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:i<4?6:0}}>
                <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0,flex:1}}><span style={{fontSize:10,color:"var(--n-400)",width:12}}>{i+1}.</span><span style={{fontSize:11,fontWeight:500,color:"var(--n-800)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</span></div>
                <span style={{fontSize:11,fontWeight:700,color:sC(c.risk||50),flexShrink:0}}>{c.risk||50}</span>
              </div>)}
            </div>
            {/* Lines coverage */}
            <div className="cs" style={{padding:"12px 14px"}}>
              <p className="lbl" style={{color:"var(--n-500)",marginBottom:8,fontSize:9}}>{t("portfolio_lines")}</p>
              {(()=>{const allLines=wlSigs.flatMap(s=>getImpsAll(s.id).map(i=>i.line));const lineCounts={};allLines.forEach(l=>{lineCounts[l]=(lineCounts[l]||0)+1});const sorted=Object.entries(lineCounts).sort((a,b)=>b[1]-a[1]).slice(0,6);return sorted.length>0?sorted.map(([l,n])=><div key={l} style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
                <span style={{fontSize:11,color:"var(--n-700)"}}>{lineLbl(l,lang)}</span>
                <span style={{fontSize:10,fontWeight:600,color:"var(--navy)"}}>{n}</span>
              </div>):<p style={{fontSize:10,color:"var(--n-400)",fontStyle:"italic"}}>{lang==="fr"?"Aucun signal":"No signals"}</p>})()}
            </div>
          </div>
        </div>}

        <div className="fi fi3" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 className="lbl" style={{color:"var(--n-500)"}}>{t("priority_feed")}</h3>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <div style={{display:"flex",borderRadius:8,overflow:"hidden",border:"1px solid var(--n-200)"}}><button className="btn" style={{padding:"4px 10px",fontSize:10,background:sortMode==="recent"?"rgba(0,114,206,.06)":"var(--bg3)",color:sortMode==="recent"?"var(--navy)":"var(--n-400)"}} onClick={()=>{setSortMode("recent");lsSet("sortMode","recent")}}>{t("sort_recent")}</button><button className="btn" style={{padding:"4px 10px",fontSize:10,background:sortMode==="important"?"rgba(0,114,206,.06)":"var(--bg3)",color:sortMode==="important"?"var(--navy)":"var(--n-400)",borderLeft:"1px solid var(--n-200)"}} onClick={()=>{setSortMode("important");lsSet("sortMode","important")}}>{t("sort_important")}</button></div>
            <div style={{display:"flex",borderRadius:8,overflow:"hidden",border:"1px solid var(--n-200)"}}><button className="btn" style={{padding:"4px 8px",background:viewMode==="grid"?"rgba(0,114,206,.06)":"var(--bg3)",color:viewMode==="grid"?"var(--navy)":"var(--n-400)"}} onClick={()=>{setViewMode("grid");lsSet("viewMode","grid")}}>▦</button><button className="btn" style={{padding:"4px 8px",background:viewMode==="list"?"rgba(0,114,206,.06)":"var(--bg3)",color:viewMode==="list"?"var(--navy)":"var(--n-400)",borderLeft:"1px solid var(--n-200)"}} onClick={()=>{setViewMode("list");lsSet("viewMode","list")}}>☰</button></div>
          </div>
        </div>
        {viewMode==="grid"?<div className="sig-grid">{wlSigs.map((s,i)=> <SigCard key={s.id||i} s={s} d={Math.min(i+1,5)}/>)}{wlSigs.length===0&&<div style={{textAlign:"center",padding:"56px 20px",gridColumn:"1/-1"}}><I.radar style={{width:40,height:40,color:"var(--n-300)",margin:"0 auto 16px",display:"block"}}/><p style={{fontSize:15,color:"var(--n-600)",marginBottom:4,fontWeight:500}}>{search||activeCat?t("no_signals_match"):t("no_signals_yet")}</p><p style={{fontSize:13,color:"var(--n-400)"}}>{search||activeCat?t("adjust_filters"):t("radar_will_update")}</p></div>}</div>
        :<div style={{display:"flex",flexDirection:"column",gap:2}}>{wlSigs.map((s,i)=>{const cat=getCat(s.cat,lang);const co=cos.find(c=>c.id===s.cid)||cos.find(c=>s.company&&(c.name.toLowerCase()===s.company.toLowerCase()||s.company.toLowerCase().includes(c.name.toLowerCase().split(" ")[0])));const imps=getImpsAll(s.id);return (
          <div key={s.id||i} className={`fi fi${Math.min(i+1,5)}`} style={{padding:"12px 16px",borderBottom:"1px solid var(--n-200)",cursor:"pointer",background:i%2===0?"transparent":"rgba(13,20,36,.4)"}} onClick={()=>setSS(s)}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
                <span className="badge" style={{background:sBg(s.imp||50),color:sT(s.imp||50),flexShrink:0}}>{s.imp||50}</span>
                <div style={{flex:1,minWidth:0}}>
                  <p style={{fontSize:12,fontWeight:600,color:"var(--n-800)",lineHeight:1.3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tx(s.title,lang)||s.company||"—"}</p>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginTop:3}}>
                    <Logo name={co?.name||s.company} sz={16} fallback={co?.logo}/>
                    <span style={{fontSize:10,color:"var(--navy)",fontWeight:500}}>{co?.name||s.company||"—"}</span>
                    <span style={{fontSize:9,color:"var(--n-400)"}}>·</span>
                    <span style={{fontSize:10,color:cat?.c||"var(--n-500)"}}>{cat?.icon} {cat?.s||""}</span>
                    {imps.length>0&&<>{imps.slice(0,2).map((im,idx)=><span key={idx} style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:LVL_BG[im.lvl]||"rgba(59,130,246,.1)",color:LVL_T[im.lvl]||"#1E40AF"}}>{lineLbl(im.line,lang)}</span>)}</>}
                  </div>
                </div>
              </div>
              <span style={{fontSize:10,color:"var(--n-400)",flexShrink:0,marginLeft:8}}>{s.at?fD(s.at):"—"}</span>
            </div>
          </div>)})}
          {wlSigs.length===0&&<div style={{textAlign:"center",padding:"56px 20px"}}><p style={{fontSize:15,color:"var(--n-600)",marginBottom:4,fontWeight:500}}>{search||activeCat?t("no_signals_match"):t("no_signals_yet")}</p></div>}
        </div>}
      </div>
    </div>);

    if(tab==="watchlist")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><h2 style={{fontSize:18,fontWeight:700,color:"var(--n-800)"}}>{t("watchlist_title")}</h2><p style={{fontSize:12,color:"var(--n-500)",marginTop:4}}>{t("watchlist_sub")}</p></div>
      <div style={{padding:"18px 20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h3 className="lbl" style={{color:"var(--navy)"}}>{t("tracked")} ({watched.length})</h3>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <div style={{display:"flex",borderRadius:8,overflow:"hidden",border:"1px solid var(--n-200)"}}>{[{k:"risk",l:t("sort_risk")},{k:"alpha",l:t("sort_alpha")},{k:"signals",l:t("sort_signals")}].map(s=><button key={s.k} className="btn" style={{padding:"4px 10px",fontSize:10,background:wlSort===s.k?"rgba(0,114,206,.06)":"var(--bg3)",color:wlSort===s.k?"var(--navy)":"var(--n-400)",borderRight:"1px solid var(--n-200)"}} onClick={()=>{setWlSort(s.k);lsSet("wlSort",s.k)}}>{s.l}</button>)}</div>
            <div style={{display:"flex",borderRadius:8,overflow:"hidden",border:"1px solid var(--n-200)"}}><button className="btn" style={{padding:"4px 8px",background:wlView==="grid"?"rgba(0,114,206,.06)":"var(--bg3)",color:wlView==="grid"?"var(--navy)":"var(--n-400)"}} onClick={()=>{setWlView("grid");lsSet("wlView","grid")}}>▦</button><button className="btn" style={{padding:"4px 8px",background:wlView==="list"?"rgba(0,114,206,.06)":"var(--bg3)",color:wlView==="list"?"var(--navy)":"var(--n-400)",borderLeft:"1px solid var(--n-200)"}} onClick={()=>{setWlView("list");lsSet("wlView","list")}}>☰</button></div>
          </div>
        </div>
        ...
        <div className="co-grid" style={{marginBottom:28}}>{sorted.map((c,i)=>{const sc=getSigs(c.id).length;return (
          <div key={c.id} className={`card fi fi${Math.min(i+1,5)} prio-${c.prio}`} style={{padding:"16px 18px",position:"relative"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:12,flex:1,minWidth:0}}><Logo name={c.name} sz={24} fallback={c.logo}/><div style={{minWidth:0}}><h4 style={{fontSize:14,fontWeight:600,color:"var(--n-800)"}}>{c.name}</h4><p style={{fontSize:12,color:"var(--n-500)",marginTop:2}}>{tx(c.sector,lang)}</p><div style={{display:"flex",gap:8,marginTop:6}}><span style={{fontSize:10,color:"var(--n-400)"}}>{sc} {sc>1?t("signals_lc"):t("signal")}</span><span className="lbl" style={{fontSize:8,color:c.prio==="primary"?"var(--navy)":c.prio==="secondary"?"#0072CE":"var(--n-400)"}}>{t(c.prio)}</span></div></div></div><SR s={c.risk} sz={40} sw={2.5}/></div>
            </button>
          </div>)})}
        </div>
        :<div style={{marginBottom:28}}>
        <div style={{display:"grid",gridTemplateColumns:"32px 1fr 80px 50px 50px 36px 22px",alignItems:"center",gap:8,padding:"6px 16px",borderBottom:"2px solid var(--n-200)"}}><span/><span className="lbl" style={{fontSize:8,color:"var(--n-400)"}}>{lang==="fr"?"ENTREPRISE":"COMPANY"}</span><span className="lbl" style={{fontSize:8,color:"var(--n-400)"}}>{lang==="fr"?"LIGNES":"LINES"}</span><span className="lbl" style={{fontSize:8,color:"var(--n-400)",textAlign:"center"}}>{lang==="fr"?"SIG.":"SIG."}</span><span className="lbl" style={{fontSize:8,color:"var(--n-400)",textAlign:"center"}}>NOTES</span><span className="lbl" style={{fontSize:8,color:"var(--n-400)",textAlign:"center"}}>{lang==="fr"?"RISQ.":"RISK"}</span><span/></div>
        {sorted.map((c,i)=>{const sc=getSigs(c.id).length;const nc=getNotes(c.id).length;const lines=getLinesAll(getSigs(c.id));return (
          <div key={c.id} className={`fi fi${Math.min(i+1,5)}`} style={{padding:"10px 16px",borderBottom:"1px solid var(--n-200)",background:i%2===0?"transparent":"rgba(13,20,36,.4)",display:"grid",gridTemplateColumns:"32px 1fr 80px 50px 50px 36px 22px",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>setSC(c.id)}>
              <Logo name={c.name} sz={28} fallback={c.logo}/>
              <div style={{minWidth:0}}>
                <h4 style={{fontSize:12,fontWeight:600,color:"var(--n-800)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.name}</h4>
                <p style={{fontSize:9,color:"var(--n-500)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginTop:1}}>{tx(c.sector,lang)}</p>
              </div>
              <div style={{display:"flex",gap:3,flexWrap:"wrap",justifyContent:"flex-start"}}>{lines.length>0?lines.slice(0,2).map(l=><span key={l} style={{fontSize:7,padding:"1px 5px",borderRadius:5,background:"rgba(96,165,250,.1)",color:"#1E40AF",whiteSpace:"nowrap"}}>{lineLbl(l,lang)}</span>):<span style={{fontSize:7,color:"var(--n-400)"}}>—</span>}</div>
              <div style={{textAlign:"center"}}><span style={{fontSize:11,color:sc>0?"#0072CE":"var(--n-400)",fontWeight:sc>0?600:400}}>{sc}</span><p style={{fontSize:7,color:"var(--n-400)",marginTop:1}}>{t("signals_lc")}</p></div>
              <div style={{textAlign:"center"}}><span style={{fontSize:11,color:nc>0?"var(--navy)":"var(--n-400)",fontWeight:nc>0?600:400}}>{nc}</span><p style={{fontSize:7,color:"var(--n-400)",marginTop:1}}>{t("notes_lc")}</p></div>
              <SR s={c.risk} sz={32} sw={2}/>
              <button style={{width:20,height:20,borderRadius:5,background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.12)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={e=>{e.stopPropagation();togW(c.id)}}><I.x style={{width:9,height:9,color:"#DC2626"}}/></button>
          </div>)})}
        </div>;})()}{watched.length===0&&<div style={{textAlign:"center",padding:"48px 20px"}}><p style={{fontSize:14,color:"var(--n-600)",fontWeight:500,marginBottom:4}}>{t("no_companies_yet")}</p><p style={{fontSize:13,color:"var(--n-400)"}}>{t("no_companies_sub")}</p></div>}
        <h3 className="lbl" style={{color:"var(--n-500)",marginBottom:10}}>{t("add_company")}</h3><input className="inp" style={{marginBottom:14}} placeholder={t("search_company")} value={addSrch} onChange={e=>{setAS(e.target.value);searchExternal(e.target.value)}}/>{(()=>{const localFiltered=cos.filter(c=>!c.prio).filter(c=>!addSrch||c.name.toLowerCase().includes(addSrch.toLowerCase())||(c.ticker||"").toLowerCase().includes(addSrch.toLowerCase())||tx(c.sector,lang).toLowerCase().includes(addSrch.toLowerCase()));return (<><div style={{display:"flex",flexDirection:"column",gap:10}}>{localFiltered.slice(0,10).map(c=>(<div key={c.id} className="card" style={{padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><button style={{flex:1,textAlign:"left",background:"none",border:"none",cursor:"pointer",minWidth:0,fontFamily:"inherit",color:"inherit"}} onClick={()=>setSC(c.id)}><h4 style={{fontSize:13,fontWeight:500,color:"var(--n-700)"}}>{c.name}</h4><p style={{fontSize:11,color:"var(--n-500)"}}>{tx(c.sector,lang)}</p></button><button className="btn bp" style={{padding:"6px 14px",fontSize:12}} onClick={()=>togW(c.id)}><I.plus/>{t("add_to_watchlist")}</button></div>))}</div>{addSrch.length>=3&&<>{extLoading&&<p style={{fontSize:12,color:"var(--n-500)",marginTop:14,textAlign:"center"}}>{t("searching")}</p>}{!extLoading&&extRes.length>0&&<><h4 className="lbl" style={{color:"var(--navy)",marginTop:18,marginBottom:10}}>{t("ext_results")}</h4><div style={{display:"flex",flexDirection:"column",gap:10}}>{extRes.map((r,i)=>(<div key={i} className="card" style={{padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",borderColor:"rgba(0,43,92,.15)"}}><div style={{flex:1,minWidth:0}}><h4 style={{fontSize:13,fontWeight:500,color:"var(--n-800)"}}>{r.name}</h4><p style={{fontSize:11,color:"var(--n-500)",marginTop:2,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{r.desc}</p></div><button className="btn bp" style={{padding:"6px 14px",fontSize:12,flexShrink:0,marginLeft:10}} onClick={()=>addExtCompany(r.name,r.desc)}><I.plus/>{lang==="fr"?"Ajouter":"Add"}</button></div>))}</div></>}{!extLoading&&extRes.length===0&&localFiltered.length===0&&addSrch.length>=3&&<p style={{fontSize:12,color:"var(--n-500)",marginTop:14,textAlign:"center"}}>{t("no_ext_results")}</p>}</>}</>)})()}
      </div>
    </div>);

    if(tab==="signals")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{fontSize:18,fontWeight:700,color:"var(--n-800)"}}>{t("all_signals")}</h2><button className="bi" style={{width:34,height:34}} onClick={()=>{setSSh(!showSearch);if(showSearch)setSrch("")}}>{showSearch?<I.x/>:<I.filter/>}</button></div>{showSearch&&<input className="inp" style={{marginBottom:12}} placeholder={t("filter_signals")} value={search} onChange={e=>setSrch(e.target.value)} autoFocus/>}<div className="hsb" style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4}}><button className={`chip ${!activeCat?"on":""}`} onClick={()=>setACat(null)}>{t("all")}</button>{CATS.map(c=>{const cc=getCat(c.id,lang);return <button key={c.id} className={`chip ${activeCat===c.id?"on":""}`} onClick={()=>setACat(activeCat===c.id?null:c.id)}>{cc?.icon} {cc?.s}</button>})}</div></div>
    ...
    if(tab==="notes")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><h2 style={{fontSize:18,fontWeight:700,color:"var(--n-800)"}}>{t("notes_title")}</h2><button className="btn bp" style={{padding:"6px 14px",fontSize:12}} onClick={()=>setSNN(true)}><I.plus/>{t("new_note")}</button></div></div>
      ...
      <div style={{padding:"18px 20px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>{upcomingMtgs.map((mtg,i)=>{const co=cos.find(c=>c.id===mtg.cid);const ml=mtgLabel(mtg);const d=new Date(mtg.date);return (
          <div key={mtg.id} className={`card fi fi${Math.min(i+1,5)}`} style={{padding:"16px 18px",borderLeft:`3px solid ${ml.c}`,position:"relative"}}>
            <button style={{position:"absolute",top:8,right:8,width:20,height:20,borderRadius:5,background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.12)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={()=>deleteMeeting(mtg.id)}><I.x style={{width:10,height:10,color:"#DC2626"}}/></button>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingRight:24}}>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  ...
                  <span className="ftag" style={{background:mtg.type==="broker"?"rgba(0,43,92,.15)":mtg.type==="autre"?"rgba(139,92,246,.15)":"rgba(96,165,250,.15)",color:mtg.type==="broker"?"var(--navy)":mtg.type==="autre"?"#5B21B6":"#1E40AF"}}>{mtg.type==="broker"?t("meeting_broker"):mtg.type==="rm"?t("meeting_rm"):mtg.type==="autre"?t("meeting_other"):t("meeting_internal")}</span>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  {co&&<><Logo name={co.name} sz={24} fallback={co.logo}/><span style={{fontSize:14,fontWeight:600,color:"var(--n-800)"}}>{co.name}</span></>}
                </div>
                <p style={{fontSize:11,color:"var(--n-500)"}}>{d.toLocaleDateString(lang==="fr"?"fr-FR":"en-GB",{weekday:"long",day:"numeric",month:"long"})} · {d.toLocaleTimeString(lang==="fr"?"fr-FR":"en-GB",{hour:"2-digit",minute:"2-digit"})}</p>
                {mtg.notes&&<p style={{fontSize:11,color:"var(--n-400)",marginTop:4,fontStyle:"italic"}}>{mtg.notes}</p>}
                {mtg.contact&&mtg.contact.name&&<div style={{marginTop:6,padding:"6px 10px",background:"rgba(139,92,246,.06)",borderRadius:6,border:"1px solid rgba(139,92,246,.12)"}}>
                  <p style={{fontSize:11,fontWeight:600,color:"#5B21B6"}}>{mtg.contact.name}{mtg.contact.role&&<span style={{fontWeight:400,color:"var(--n-500)"}}> — {mtg.contact.role}</span>}</p>
                  <div style={{display:"flex",gap:10,marginTop:3}}>{mtg.contact.phone&&<span style={{fontSize:10,color:"var(--n-500)"}}>📞 {mtg.contact.phone}</span>}{mtg.contact.email&&<span style={{fontSize:10,color:"var(--n-500)"}}>✉️ {mtg.contact.email}</span>}</div>
                </div>}
              </div>
            </div>
            <div style={{display:"flex",gap:6,marginTop:10}}>
              <button className="btn" style={{flex:1,padding:"6px",fontSize:11,background:"var(--bg3)",color:"var(--n-600)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)"}} onClick={()=>{setBC(mtg.cid);setSB(true);setCopied(false)}}><I.brief style={{width:12,height:12}}/>{t("generate")}</button>
              <button className="btn" style={{flex:1,padding:"6px",fontSize:11,background:"var(--bg3)",color:"var(--n-600)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)"}} onClick={()=>exportICS(mtg)}><I.download style={{width:12,height:12}}/>{t("export_cal")}</button>
            </div>
          </div>)})}</div></>}

        {/* Past meetings */}
        {pastMtgs.length>0&&<><h3 className="lbl" style={{color:"var(--n-400)",marginBottom:10}}>{t("past_meetings")} ({pastMtgs.length})</h3>
        <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:24}}>{pastMtgs.slice(0,5).map(mtg=>{const co=cos.find(c=>c.id===mtg.cid);return (
          <div key={mtg.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 14px",borderBottom:"1px solid var(--n-200)",opacity:.6}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>{co&&<Logo name={co.name} sz={22} fallback={co.logo}/>}<span style={{fontSize:12,color:"var(--n-600)"}}>{co?.name||"—"}</span><span style={{fontSize:10,color:"var(--n-400)"}}>{fD(mtg.date)}</span></div>
            <button style={{width:18,height:18,borderRadius:4,background:"rgba(239,68,68,.06)",border:"1px solid rgba(239,68,68,.1)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}} onClick={()=>deleteMeeting(mtg.id)}><I.x style={{width:9,height:9,color:"#DC2626"}}/></button>
          </div>)})}</div></>}

        {meetings.length===0&&<div style={{textAlign:"center",padding:"32px 20px",marginBottom:24}}><I.calendar style={{width:32,height:32,color:"var(--n-300)",margin:"0 auto 12px",display:"block"}}/><p style={{fontSize:13,color:"var(--n-500)",marginBottom:12}}>{t("no_meetings")}</p><button className="btn bp" style={{padding:"8px 18px",fontSize:12}} onClick={()=>setSNM(true)}><I.plus/>{t("add_meeting")}</button></div>}

        {/* Company list for quick brief */}
        <div className="dv" style={{marginBottom:18}}/>
        <h3 className="lbl" style={{color:"var(--n-500)",marginBottom:12}}>{t("brief_title")} — {t("generate")}</h3>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>{[...watched].sort((a,b)=>a.name.localeCompare(b.name)).map((c,i)=>{const sc=getSigs(c.id).length;const nc=getNotes(c.id).length;const lb=getLastBriefDate(c.id);const ns=getNewSignalsSinceLastBrief(c.id).length;const bh=getBriefHistory(c.id).length;const dos=getDossier(c.id);return (<div key={c.id} className={`card fi fi${Math.min(i+1,5)}`} style={{padding:"16px 18px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,flex:1,cursor:"pointer"}} onClick={()=>setSC(c.id)}><Logo name={c.name} sz={24} fallback={c.logo}/><div><h4 style={{fontSize:13,fontWeight:600,color:"var(--n-800)"}}>{c.name}</h4><p style={{fontSize:11,color:"var(--n-500)",marginTop:2}}>{sc} {sc>1?t("signals_lc"):t("signal")} · {nc} {nc>1?t("notes_lc"):t("note_lc")}{lb&&<span style={{color:"var(--n-400)"}}> · Brief {fD(lb)}</span>}{ns>0&&<span style={{color:"#16A34A",fontWeight:600}}> · {ns}</span>}</p></div></div>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <button className="btn" style={{padding:"5px 10px",fontSize:10,background:dos?"rgba(139,92,246,.1)":"var(--bg3)",color:dos?"#5B21B6":"var(--n-400)",border:`1px solid ${dos?"rgba(139,92,246,.2)":"var(--n-200)"}`,borderRadius:6}} onClick={()=>openDossier(c.id)}>{dos?"":"+"} {lang==="fr"?"Dossier":"File"}</button>
    ...
    if(tab==="settings")return (<div style={{paddingBottom:100}}>
      <div className="hdr"><h2 style={{fontSize:18,fontWeight:700,color:"var(--n-800)"}}>{t("settings_title")}</h2></div>
      <div style={{padding:"24px 20px"}}>
        <h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{t("profile")}</h3>
        <div className="card" style={{padding:"18px",marginBottom:28}}><div style={{display:"flex",alignItems:"center",gap:14}}><div className="mono" style={{width:44,height:44,fontSize:16,background:"linear-gradient(135deg,var(--navy),rgba(0,43,92,.6))",color:"var(--n-100)"}}>AS</div><div><p style={{fontSize:14,fontWeight:600,color:"var(--n-800)"}}>Anne-Sophie</p><p style={{fontSize:12,color:"var(--n-500)",marginTop:2}}>Senior Account Manager — Financial Lines France</p></div></div></div>
        <h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{t("language")}</h3>
        ...
        <h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{t("preferred_lines")}</h3>
        <div className="card" style={{padding:"16px 18px",marginBottom:28}}><p style={{fontSize:12,color:"var(--n-500)",marginBottom:12}}>{t("preferred_lines_sub")}</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{Object.entries(LINES).map(([k])=>{const checked=selLines.includes(k);return (<label key={k} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"4px 0"}} onClick={()=>togLine(k)}><div style={{width:16,height:16,borderRadius:4,border:`2px solid ${checked?"var(--navy)":"var(--n-300)"}`,background:checked?"rgba(0,114,206,.06)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .2s",flexShrink:0}}>{checked&&<span style={{color:"var(--navy)",fontSize:10,fontWeight:700}}>✓</span>}</div><span style={{fontSize:12,color:checked?"var(--n-800)":"var(--n-600)",transition:"color .2s"}}>{lineLbl(k,lang)}</span></label>)})}</div></div>
        <h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{t("notifications")}</h3>
        <div className="card" style={{padding:"16px 18px",marginBottom:28}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}} onClick={()=>{setAutoRefresh(!autoRefresh);savePrefsDB({auto_refresh:!autoRefresh})}}><span style={{fontSize:13,color:"var(--n-700)"}}>{lang==="fr"?"Rafraîchissement automatique":"Auto-refresh"}</span><div style={{width:36,height:20,borderRadius:10,background:autoRefresh?"var(--navy)":"var(--n-300)",padding:2,cursor:"pointer",transition:"background .2s"}}><div style={{width:16,height:16,borderRadius:8,background:autoRefresh?"white":"var(--n-500)",marginLeft:autoRefresh?16:0,transition:"margin-left .2s"}}/></div></div>
          {autoRefresh&&<div style={{marginBottom:14}}><p className="lbl" style={{color:"var(--n-400)",marginBottom:8,fontSize:9}}>{lang==="fr"?"Fréquence de mise à jour":"Update frequency"}</p><div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}><input type="number" className="inp" style={{width:70,textAlign:"center",padding:"8px 10px"}} value={refreshVal} min={1} onChange={e=>applyRefresh(e.target.value,refreshUnit)}/><div style={{display:"flex",borderRadius:"var(--rs)",overflow:"hidden",border:"1px solid var(--n-200)"}}>{[{k:"m",l:"min"},{k:"h",l:lang==="fr"?"heure(s)":"hour(s)"},{k:"j",l:lang==="fr"?"jour(s)":"day(s)"}].map(u=><button key={u.k} className="btn" style={{padding:"8px 14px",fontSize:12,background:refreshUnit===u.k?"rgba(0,114,206,.06)":"var(--bg3)",color:refreshUnit===u.k?"var(--navy)":"var(--n-500)",borderRight:"1px solid var(--n-200)"}} onClick={()=>applyRefresh(refreshVal,u.k)}>{u.l}</button>)}</div></div><p style={{fontSize:10,color:"var(--n-400)",marginBottom:4}}>{lang==="fr"?"Suggestions :":"Suggestions:"}</p><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[{v:15,u:"m",l:"15 min"},{v:30,u:"m",l:"30 min"},{v:1,u:"h",l:"1h"},{v:2,u:"h",l:"2h"},{v:4,u:"h",l:"4h"},{v:1,u:"j",l:lang==="fr"?"1 jour":"1 day"}].map(s=><button key={s.l} className="btn" style={{padding:"4px 10px",fontSize:10,borderRadius:12,background:"var(--bg3)",color:"var(--n-500)",border:"1px solid var(--n-200)"}} onClick={()=>{setRefreshVal(s.v);setRefreshUnit(s.u);applyRefresh(s.v,s.u)}}>{s.l}</button>)}</div></div>}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,color:"var(--n-700)"}}>{t("critical_alerts")}</span><div style={{width:36,height:20,borderRadius:10,background:lsGet("critAlerts",false)?"var(--navy)":"var(--n-300)",padding:2,cursor:"pointer",transition:"background .2s"}} onClick={()=>{const next=!lsGet("critAlerts",false);lsSet("critAlerts",next);if(next&&typeof Notification!=="undefined"&&Notification.permission!=="granted"){Notification.requestPermission().then(p=>{if(p!=="granted"){lsSet("critAlerts",false);showT(lang==="fr"?"Notifications refusées par le navigateur":"Notifications denied by browser")}else{showT(lang==="fr"?"Alertes critiques activées":"Critical alerts enabled")}})}else if(next){showT(lang==="fr"?"Alertes critiques activées":"Critical alerts enabled")}else{showT(lang==="fr"?"Alertes critiques désactivées":"Critical alerts disabled")}}}><div style={{width:16,height:16,borderRadius:8,background:lsGet("critAlerts",false)?"white":"var(--n-500)",marginLeft:lsGet("critAlerts",false)?16:0,transition:"margin-left .2s"}}/></div></div>
          <p style={{fontSize:11,color:"var(--n-400)",marginTop:12}}>{t("notif_coming")}</p>
        </div>
        <h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{lang==="fr"?"IA Locale (Ollama)":"Local AI (Ollama)"}</h3>
        <div className="card" style={{padding:"16px 18px",marginBottom:24}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <span style={{fontSize:13,color:"var(--n-700)"}}>{lang==="fr"?"Enrichissement IA":"AI Enrichment"}</span>
            <div style={{width:36,height:20,borderRadius:10,background:ollamaEnabled?"var(--navy)":"var(--n-300)",padding:2,cursor:"pointer",transition:"background .2s"}} onClick={()=>{const v=!ollamaEnabled;setOllamaEnabled(v);lsSet("ollamaEnabled",v);showT(v?(lang==="fr"?"IA activée":"AI enabled"):(lang==="fr"?"IA désactivée":"AI disabled"))}}><div style={{width:16,height:16,borderRadius:8,background:ollamaEnabled?"white":"var(--n-500)",marginLeft:ollamaEnabled?16:0,transition:"margin-left .2s"}}/></div>
          </div>
          {ollamaEnabled&&<>
            <div style={{marginBottom:12}}><label className="lbl" style={{color:"var(--n-400)",display:"block",marginBottom:6,fontSize:9}}>{lang==="fr"?"Adresse Ollama":"Ollama URL"}</label><input className="inp" value={ollamaUrl} onChange={e=>{setOllamaUrl(e.target.value);lsSet("ollamaUrl",e.target.value)}} placeholder="http://localhost:11434"/></div>
            <div style={{marginBottom:12}}><label className="lbl" style={{color:"var(--n-400)",display:"block",marginBottom:6,fontSize:9}}>{lang==="fr"?"Modèle":"Model"}</label><input className="inp" value={ollamaModel} onChange={e=>{setOllamaModel(e.target.value);lsSet("ollamaModel",e.target.value)}} placeholder="mistral"/></div>
            <button className="btn" style={{width:"100%",padding:"8px",fontSize:12,background:"var(--bg3)",color:"var(--n-600)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)"}} onClick={async()=>{try{const r=await fetch(ollamaUrl+"/api/tags");if(r.ok){const d=await r.json();showT(`${lang==="fr"?"Connecté":"Connected"} — ${(d.models||[]).length} ${lang==="fr"?"modèle(s)":"model(s)"}`)}else showT(lang==="fr"?"Erreur de connexion":"Connection error")}catch(e){showT(lang==="fr"?"Ollama non accessible. Vérifiez qu'il est lancé.":"Ollama unreachable. Check it's running.")}}}>{lang==="fr"?"Tester la connexion":"Test connection"}</button>
            {enriching&&<p style={{fontSize:11,color:"var(--navy)",marginTop:10,textAlign:"center"}}>{lang==="fr"?"Enrichissement en cours...":"Enriching signals..."}</p>}
          </>}
        </div>
        <h3 className="lbl" style={{color:"var(--navy)",marginBottom:14}}>{lang==="fr"?"Scores & Critères":"Scores & Criteria"}</h3>
        <div className="card" style={{padding:"16px 18px",marginBottom:24}}>
          <p className="lbl" style={{color:"var(--n-400)",marginBottom:14,fontSize:9}}>{lang==="fr"?"Chaque signal est classé selon les mots-clés détectés. Personnalisez les critères de chaque niveau.":"Each signal is classified by detected keywords. Customize criteria for each level."}</p>
          {[
            ...
            {k:"high",c:"#D97706",l:lang==="fr"?"Élevé":"High",d:lang==="fr"?"Signal significatif. Discussion courtier/RM.":"Significant. Broker/RM discussion.",range:`${scoreThresholds.high}–${scoreThresholds.critical-1}`},
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:12,fontWeight:700,color:x.c}}>{x.l}</span><span style={{fontSize:9,color:"var(--n-400)",background:"var(--bg3)",padding:"1px 6px",borderRadius:8}}>{x.range}</span></div>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <p style={{fontSize:9,color:"var(--n-500)",marginBottom:8}}>{x.d}</p>
            ...
            {editingKwLevel===x.k?<div style={{position:"relative"}}>
              <div style={{display:"flex",gap:6,marginBottom:4}}>
                ...
                <button className="btn" style={{padding:"4px 8px",fontSize:10,color:"var(--n-400)",background:"var(--bg3)",border:"1px solid var(--n-200)",borderRadius:6}} onClick={()=>{setNewKw("");setEditingKwLevel(null)}}>✕</button>
              </div>
              <div style={{maxHeight:180,overflow:"auto",display:"flex",flexDirection:"column",gap:2}}>
                {getKwSuggestions(newKw).map((s,i)=><button key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 10px",background:"var(--bg3)",border:"1px solid var(--n-200)",borderRadius:6,cursor:"pointer",fontFamily:"inherit",color:"inherit",textAlign:"left"}} onClick={()=>{setScoreKeywords(p=>{const n={...p,[x.k]:[...p[x.k],s.kw]};lsSet("scoreKeywords",n);return n});setNewKw("")}}>
                  <span style={{fontSize:11,color:"var(--n-700)"}}>{s.kw}</span>
                  <span style={{fontSize:8,color:"var(--n-400)",marginLeft:8}}>{s.cat}</span>
                </button>)}
                {getKwSuggestions(newKw).length===0&&newKw&&<p style={{fontSize:10,color:"var(--n-400)",padding:"4px 10px",fontStyle:"italic"}}>{lang==="fr"?"Appuyez Entrée pour ajouter ce critère personnalisé":"Press Enter to add this custom keyword"}</p>}
              </div>
            </div>
            :<button style={{fontSize:10,color:"var(--n-400)",background:"none",border:`1px dashed ${x.c}30`,borderRadius:6,padding:"3px 10px",cursor:"pointer"}} onClick={()=>setEditingKwLevel(x.k)}>+ {lang==="fr"?"Ajouter un critère":"Add keyword"}</button>}
          </div>)}
          <div style={{display:"flex",justifyContent:"flex-end"}}><button className="btn" style={{padding:"4px 12px",fontSize:10,color:"var(--n-400)",background:"var(--bg3)",border:"1px solid var(--n-200)",borderRadius:6}} onClick={()=>{setScoreKeywords(defaultKeywords);lsSet("scoreKeywords",defaultKeywords);showT(lang==="fr"?"Critères réinitialisés":"Criteria reset")}}>{lang==="fr"?"Réinitialiser":"Reset defaults"}</button></div>
        </div>
        <h3 className="lbl" style={{color:"var(--n-500)",marginBottom:14}}>{t("about")}</h3>
        <div className="card" style={{padding:"16px 18px",marginBottom:16}}>
          <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0"}}><span style={{fontSize:13,color:"var(--n-600)"}}>{t("version")}</span><span style={{fontSize:13,color:"var(--n-500)"}}>1.0.0</span></div>
          <div className="dv" style={{margin:"8px 0"}}/>
          <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0"}}><span style={{fontSize:13,color:"var(--n-600)"}}>{lang==="fr"?"Base de données":"Database"}</span><span style={{fontSize:13,color:sbOk?"#166534":"var(--n-400)"}}>{sbOk?(lang==="fr"?"Connectée":"Connected"):(lang==="fr"?"Hors-ligne":"Offline")}</span></div>
          {lastRefresh&&<><div className="dv" style={{margin:"8px 0"}}/><div style={{display:"flex",justifyContent:"space-between",padding:"6px 0"}}><span style={{fontSize:13,color:"var(--n-600)"}}>{lang==="fr"?"Dernière mise à jour":"Last refresh"}</span><span style={{fontSize:13,color:"var(--n-500)"}}>{new Date(lastRefresh).toLocaleTimeString(lang==="fr"?"fr-FR":"en-GB",{hour:"2-digit",minute:"2-digit"})}</span></div></>}
        </div>
        <p style={{textAlign:"center",fontSize:11,color:"var(--n-400)",marginBottom:28}}>© 2026 AIG FL Intelligence — AIG</p>
        <button className="btn" style={{width:"100%",background:"rgba(239,68,68,.08)",color:"#991B1B",border:"1px solid rgba(239,68,68,.15)",borderRadius:"var(--rs)"}} onClick={()=>{try{["step","tab","lang","selLines","autoRefresh","watchPrios","watchExtras","userNotes"].forEach(k=>localStorage.removeItem("signalis_"+k))}catch(e){}setStep("login");setTab("dashboard");setLoginEm("");setLoginPw("");setLoginErr(false)}}><I.logout/>{t("sign_out")}</button>
      </div>
    </div>);
    return null;
  };
  ...
  return (<>
    ...
    {editingDossier&&<div className="bsbg" onClick={()=>setEditingDossier(null)}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,paddingTop:8}}><h3 style={{fontSize:18,fontWeight:600,color:"var(--n-800)"}}>{lang==="fr"?"Dossier client":"Client file"}</h3><button className="bi" style={{width:32,height:32}} onClick={()=>setEditingDossier(null)}><I.x/></button></div>
      {(()=>{const co=cos.find(c=>c.id===editingDossier);return co?<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--bg3)",borderRadius:"var(--rs)",marginBottom:16,border:"1px solid var(--n-200)"}}><Logo name={co.name} sz={28} fallback={co.logo}/><div><span style={{fontSize:13,fontWeight:600,color:"var(--n-800)"}}>{co.name}</span><p style={{fontSize:10,color:"var(--n-500)"}}>{tx(co.sector,lang)}</p></div></div>:null})()}
      <p style={{fontSize:10,color:"var(--n-400)",marginBottom:14}}>{lang==="fr"?"Ces informations enrichiront les prochains briefs de réunion.":"This information will enrich future meeting briefs."}</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{lang==="fr"?"Courtier référent":"Lead broker"}</label><input className="inp" value={dossierDraft.broker} onChange={e=>setDossierDraft(p=>({...p,broker:e.target.value}))} placeholder={lang==="fr"?"Nom du courtier...":"Broker name..."}/></div>
        <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{lang==="fr"?"Risk Manager":"Risk Manager"}</label><input className="inp" value={dossierDraft.rm} onChange={e=>setDossierDraft(p=>({...p,rm:e.target.value}))} placeholder={lang==="fr"?"Nom du RM...":"RM name..."}/></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{lang==="fr"?"Date de renouvellement":"Renewal date"}</label><input className="inp" type="date" value={dossierDraft.renewal} onChange={e=>setDossierDraft(p=>({...p,renewal:e.target.value}))}/></div>
        <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{lang==="fr"?"Prime estimée":"Estimated premium"}</label><input className="inp" value={dossierDraft.premium} onChange={e=>setDossierDraft(p=>({...p,premium:e.target.value}))} placeholder="ex: 850K€"/></div>
      </div>
      ...
      {clientDossiers[editingDossier]?.updatedAt&&<p style={{fontSize:9,color:"var(--n-400)",marginBottom:10,fontStyle:"italic"}}>{lang==="fr"?"Dernière mise à jour":"Last updated"}: {fD(clientDossiers[editingDossier].updatedAt)}</p>}
    ...
    {showNewMeeting&&<div className="bsbg" onClick={()=>{if(mtgDictating)stopMtgDict();setSNM(false)}}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,paddingTop:8}}><h3 style={{fontSize:18,fontWeight:600,color:"var(--n-800)"}}>{t("add_meeting")}</h3><button className="bi" style={{width:32,height:32}} onClick={()=>{if(mtgDictating)stopMtgDict();setSNM(false)}}><I.x/></button></div>
      ...
      <div style={{marginBottom:16}}><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:8}}>{t("meeting_date")}</label><input className="inp" type="datetime-local" value={mtgDate} onChange={e=>setMtgDate(e.target.value)}/></div>
      ...
      {mtgType==="autre"&&<div style={{padding:"14px 16px",background:"var(--bg3)",borderRadius:"var(--rs)",border:"1px solid var(--n-200)",marginBottom:16}}>
        <div style={{marginBottom:12,position:"relative"}}><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{t("contact_name")}</label><input className="inp" value={mtgContactName} onChange={e=>{setMtgCN(e.target.value);searchContacts(e.target.value)}} placeholder={lang==="fr"?"Nom et prénom...":"Full name..."}/>
          {contactSuggestions.length>0&&<div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:10,background:"var(--bg2)",border:"1px solid var(--n-200)",borderRadius:"0 0 8px 8px",maxHeight:160,overflow:"auto"}}>{contactSuggestions.map(c=><button key={c.id} style={{width:"100%",padding:"8px 12px",background:"none",border:"none",borderBottom:"1px solid var(--n-200)",cursor:"pointer",textAlign:"left",fontFamily:"inherit",color:"inherit"}} onClick={()=>{setMtgCN(c.name);setMtgCP(c.phone||"");setMtgCE(c.email||"");setMtgCR(c.role||"");setCSugg([])}}><p style={{fontSize:12,fontWeight:600,color:"var(--n-800)"}}>{c.name}</p><p style={{fontSize:10,color:"var(--n-500)"}}>{[c.role,c.company,c.phone].filter(Boolean).join(" · ")}</p>{c.history&&c.history.length>0&&<p style={{fontSize:8,color:"var(--n-400)",marginTop:2}}>📝 {lang==="fr"?"Modifié le":"Modified"} {new Date(c.updatedAt).toLocaleDateString(lang==="fr"?"fr-FR":"en-GB")}</p>}</button>)}</div>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{t("contact_phone")}</label><input className="inp" type="tel" value={mtgContactPhone} onChange={e=>setMtgCP(e.target.value)} placeholder="06 12 34 56 78"/></div>
          <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{t("contact_email")}</label><input className="inp" type="email" value={mtgContactEmail} onChange={e=>setMtgCE(e.target.value)} placeholder="prenom.nom@..."/></div>
        </div>
        <div><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:6,fontSize:9}}>{t("contact_role")}</label><input className="inp" value={mtgContactRole} onChange={e=>setMtgCR(e.target.value)} placeholder={lang==="fr"?"Ex : Directeur des risques, Courtier senior...":"E.g.: Risk Director, Senior Broker..."}/></div>
      </div>}
      <div style={{marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><label className="lbl" style={{color:"var(--n-500)"}}>{t("meeting_notes")}</label><button className="btn" style={{padding:"4px 12px",fontSize:11,borderRadius:16,background:mtgDictating?"rgba(239,68,68,.15)":"rgba(0,43,92,.1)",color:mtgDictating?"#991B1B":"var(--navy)",border:`1px solid ${mtgDictating?"rgba(239,68,68,.3)":"rgba(0,43,92,.2)"}`}} onClick={mtgDictating?stopMtgDict:startMtgDict}>{mtgDictating?<><div style={{width:6,height:6,borderRadius:"50%",background:"#DC2626",animation:"pd 1s ease-in-out infinite",marginRight:4}}/>{lang==="fr"?"Arrêter":"Stop"}</>:<><I.mic style={{width:14,height:14}}/>{lang==="fr"?"Dicter":"Dictate"}</>}</button></div>{mtgDictating&&<p style={{fontSize:10,color:"#991B1B",marginBottom:6}}>{lang==="fr"?"Parlez, la note se rédige...":"Speak, the note is being written..."}</p>}<textarea className="inp" placeholder={lang==="fr"?"Points à aborder, contexte...":"Topics to discuss, context..."} value={mtgNotes} onChange={e=>setMtgNotes(e.target.value)} rows={3} style={{borderColor:mtgDictating?"rgba(239,68,68,.3)":"var(--n-200)"}}/></div>
    ...
    {showNewNote&&<div className="bsbg" onClick={()=>{if(noteDictating)stopNoteDict();setSNN(false)}}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,paddingTop:8}}><h3 style={{fontSize:18,fontWeight:600,color:"var(--n-800)"}}>{t("new_note_title")}</h3><button className="bi" style={{width:32,height:32}} onClick={()=>{if(noteDictating)stopNoteDict();setSNN(false)}}><I.x/></button></div>
      <div style={{marginBottom:16}}><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:8}}>{t("company_optional")}</label><select className="inp" value={nComp} onChange={e=>setNC(e.target.value)} style={{appearance:"auto"}}><option value="">{t("general")}</option>{cos.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
      <div style={{marginBottom:16}}><label className="lbl" style={{color:"var(--n-500)",display:"block",marginBottom:8}}>{t("tag_label")}</label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{["observation","hypothesis","action","question","decision"].map(tg=><button key={tg} className={`chip ${nTag===tg?"on":""}`} onClick={()=>setNTg(tg)}>{noteTagLbl(tg,t)}</button>)}</div></div>
      <div style={{marginBottom:20}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><label className="lbl" style={{color:"var(--n-500)"}}>{t("note_label")}</label><button className="btn" style={{padding:"4px 12px",fontSize:11,borderRadius:16,background:noteDictating?"rgba(239,68,68,.15)":"rgba(0,43,92,.1)",color:noteDictating?"#991B1B":"var(--navy)",border:`1px solid ${noteDictating?"rgba(239,68,68,.3)":"rgba(0,43,92,.2)"}`}} onClick={noteDictating?stopNoteDict:startNoteDict}>{noteDictating?<><div style={{width:6,height:6,borderRadius:"50%",background:"#DC2626",animation:"pd 1s ease-in-out infinite",marginRight:4}}/>{t("dict_stop")}</>:<><I.mic style={{width:14,height:14}}/>{t("dict_start")}</>}</button></div>{noteDictating&&<p style={{fontSize:10,color:"#991B1B",marginBottom:6}}>{t("dict_listening")}</p>}<textarea className="inp" placeholder={t("note_placeholder")} value={nText} onChange={e=>setNT(e.target.value)} rows={4} style={{borderColor:noteDictating?"rgba(239,68,68,.3)":"var(--n-200)"}}/></div>
    ...
    {showDigest&&<div className="bsbg" onClick={()=>setSD(false)}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,paddingTop:8}}><h3 style={{fontSize:18,fontWeight:600,color:"var(--n-800)"}}>{t("digest_full_title")}</h3><button className="bi" style={{width:32,height:32}} onClick={()=>setSD(false)}><I.x/></button></div>
      <p style={{fontSize:12,color:"var(--n-500)",marginBottom:4}}>{t("digest_full_sub")}</p>
      <p style={{fontSize:10,color:"var(--n-400)",marginBottom:16}}>{fFull()} {lastRefresh?`· ${lang==="fr"?"MAJ":"Updated"} ${new Date(lastRefresh).toLocaleTimeString(lang==="fr"?"fr-FR":"en-GB",{hour:"2-digit",minute:"2-digit"})}`:""}</p>
      <div className="aline" style={{marginBottom:18}}/>
      {/* Stats bar */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>{[{l:t("watchlist_label"),v:watched.length,c:"var(--navy)"},{l:t("signals_lc"),v:liveSigs.length,c:"#0072CE"},{l:t("critical_lbl"),v:liveSigs.filter(s=>s.imp>=scoreThresholds.critical).length,c:"#DC2626"}].map(x=><div key={x.l} className="cs" style={{textAlign:"center",padding:"10px"}}><p style={{fontSize:20,fontWeight:700,color:x.c}}>{x.v}</p><p className="lbl" style={{color:"var(--n-500)",marginTop:3,fontSize:9}}>{x.l}</p></div>)}</div>
      {/* Risk overview — top companies */}
      {watched.length>0&&<><h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("risk_overview")}</h4><div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>{watched.slice(0,8).map(co=>{const coSigs=liveSigs.filter(s=>{const n=s.company||"";return n.toLowerCase()===co.name.toLowerCase()||n.toLowerCase().includes(co.name.toLowerCase().split(" ")[0])});return (<div key={co.id} className="cs" style={{padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}><Logo name={co.name} sz={26} fallback={co.logo}/><div style={{minWidth:0}}><p style={{fontSize:12,fontWeight:600,color:"var(--n-800)"}}>{co.name}</p><p style={{fontSize:10,color:"var(--n-500)",marginTop:1}}>{coSigs.length} {coSigs.length>1?t("signals_lc"):t("signal")}</p></div></div><SR s={co.risk} sz={32} sw={2}/></div>)})}</div></>}
      {/* Recent signals timeline */}
      <h4 className="lbl" style={{color:"var(--navy)",marginBottom:10}}>{t("recent_activity")}</h4>
      {liveSigs.length>0?<div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>{liveSigs.slice(0,10).map((s,idx)=>{const cat=getCat(s.cat,lang);const co=cos.find(c=>{const n=s.company||"";return c.name.toLowerCase()===n.toLowerCase()||n.toLowerCase().includes(c.name.toLowerCase().split(" ")[0])});return (<div key={s.id||idx} className="card" style={{padding:"12px 16px",cursor:"pointer"}} onClick={()=>{setSS(s);setSD(false)}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:13}}>{cat?.icon||"📋"}</span>{co&&<span style={{fontSize:11,fontWeight:500,color:"var(--navy)"}}>{co.name}</span>}{!co&&s.company&&<span style={{fontSize:11,fontWeight:500,color:"var(--navy)"}}>{s.company}</span>}</div><span className="badge" style={{background:sBg(s.imp||50),color:sT(s.imp||50)}}>{s.imp||50}</span></div><p style={{fontSize:12,color:"var(--n-800)",lineHeight:1.4}}>{tx(s.title,lang)||s.company||"—"}</p><p style={{fontSize:10,color:"var(--n-400)",marginTop:6}}>{tx(s.src||s.source,lang)||"Yahoo Finance"} · {s.at?fD(s.at):"—"}</p></div>)})}</div>:<div style={{textAlign:"center",padding:"32px 16px"}}><p style={{fontSize:13,color:"var(--n-500)",marginBottom:8}}>{t("no_activity")}</p><button className="btn bp" style={{padding:"8px 18px",fontSize:12}} onClick={()=>{setSD(false);refreshSignals()}}><I.refresh/>{lang==="fr"?"Lancer la veille":"Start monitoring"}</button></div>}
      {/* Copy digest */}
      {liveSigs.length>0&&<button className="btn bp" style={{width:"100%",height:46}} onClick={()=>{const header=`${t("digest_full_title").toUpperCase()}\n${fFull()}\n\n`;const stats=`${watched.length} ${t("companies_monitored")} · ${liveSigs.length} ${t("signal_count")}\n\n`;const sigs=liveSigs.slice(0,15).map(s=>`• [${s.imp}] ${s.company||""}: ${tx(s.title,lang)}`).join("\n");const full=`${header}${stats}${t("recent_activity").toUpperCase()}\n${sigs}\n\n— AIG FL Intelligence`;navigator.clipboard?.writeText(full);showT(t("copied_clipboard"))}}><I.copy/>{t("copy_digest")}</button>}
    </div></div>}
    {/* Recording overlay */}
    {showRec&&<div className="bsbg" onClick={()=>{if(!isRec&&!recProcessing){setShowRec(false);setTranscript("")}}}><div className="bsm" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"center",marginBottom:6}}><div style={{width:40,height:4,borderRadius:2,background:"var(--n-300)"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,paddingTop:8}}><h3 style={{fontSize:18,fontWeight:600,color:"var(--n-800)"}}>{t("rec_title")}</h3>{!isRec&&!recProcessing&&<button className="bi" style={{width:32,height:32}} onClick={()=>{setShowRec(false);setTranscript("")}}><I.x/></button>}</div>
      <p style={{fontSize:12,color:"var(--n-500)",marginBottom:20}}>{t("rec_sub")}</p>
      {cos.find(c=>c.id===recCid)&&<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--bg3)",borderRadius:"var(--rs)",marginBottom:20,border:"1px solid var(--n-200)"}}><Logo name={cos.find(c=>c.id===recCid)?.name} sz={28} fallback={cos.find(c=>c.id===recCid)?.logo}/><span style={{fontSize:13,fontWeight:600,color:"var(--n-800)"}}>{cos.find(c=>c.id===recCid)?.name}</span></div>}
      {/* Transcript area */}
      <div style={{background:"var(--bg3)",border:"1px solid var(--n-200)",borderRadius:"var(--rs)",padding:"16px",minHeight:120,maxHeight:280,overflowY:"auto",marginBottom:20}}>
        {recProcessing?<div style={{textAlign:"center",padding:"32px 0"}}><div className="pd" style={{margin:"0 auto 12px"}}/><p style={{fontSize:13,color:"var(--navy)"}}>{t("rec_processing")}</p></div>
        :isRec?<>{transcript?<p style={{fontSize:13,color:"var(--n-700)",lineHeight:1.65,whiteSpace:"pre-wrap"}}>{transcript}</p>:<p style={{fontSize:13,color:"var(--n-400)",fontStyle:"italic"}}>{lang==="fr"?"En écoute…":"Listening…"}</p>}<div style={{display:"flex",alignItems:"center",gap:8,marginTop:12}}><div style={{width:8,height:8,borderRadius:"50%",background:"#DC2626",animation:"pd 1s ease-in-out infinite"}}/><span style={{fontSize:11,color:"#991B1B"}}>{t("rec_recording")}</span></div></>
        :transcript?<p style={{fontSize:13,color:"var(--n-700)",lineHeight:1.65,whiteSpace:"pre-wrap"}}>{transcript}</p>
        :<p style={{fontSize:13,color:"var(--n-400)",textAlign:"center",padding:"24px 0"}}>{lang==="fr"?"Appuyez sur le bouton pour démarrer":"Press the button to start"}</p>}
      </div>
      {/* Controls */}
      {recProcessing?null
      :isRec?<button className="btn" style={{width:"100%",height:50,background:"rgba(239,68,68,.12)",color:"#991B1B",border:"1px solid rgba(239,68,68,.2)",borderRadius:"var(--rs)",fontSize:14,fontWeight:600}} onClick={stopRec}><I.stop/>{t("rec_stop")}</button>
      :transcript?<div style={{display:"flex",gap:10}}><button className="btn" style={{flex:1,height:46,background:"var(--bg3)",color:"var(--n-600)",border:"1px solid var(--n-300)",borderRadius:"var(--rs)"}} onClick={()=>{setTranscript("");startRec()}}><I.mic/>{lang==="fr"?"Recommencer":"Restart"}</button><button className="btn bp" style={{flex:1,height:46}} onClick={stopRec}><I.check/>{lang==="fr"?"Enregistrer le résumé":"Save summary"}</button></div>
